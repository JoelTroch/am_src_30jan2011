...
//#define MAIN_COLOR	98, 60, 30
//#define MAIN_COLOR_PRESSED	58, 37, 19
...
/*
		...
		button2->addActionSignal( new CMenuHandler_StringCommand("silencer") );
			button2->setImage(vgui_LoadTGA("gfx/vgui/HUD/640_base_left.tga"));

		button2->setPos(CVAR_GET_FLOAT( "vector_x" ), CVAR_GET_FLOAT( "vector_y" ) );
*/

/*
		if ( CVAR_GET_FLOAT( "m4a1_silencer" ) )
		CONPRINT("Silencer TRUE\n" );
		else
		CONPRINT("Silencer FALSE\n" );
*/
	/*			
	bool g_DontDrawWorld = false;

	CCheckButton2* pSwitch = new CCheckButton2();
	pSwitch->setParent(panel);
//	pSwitch->setFont(pNormalFont);
...
CONPRINT("TRUE\n" );
		...
		/*
		int _gap = 0;
		for (const char *ch = errorString; *ch != 0; ch++)
		{
			int a, b, c;

			if( *ch != '\n' )
			pNormalFont->getCharABCwide(*ch, a, b, c);
			_gap += (a + b + c);
		}

		_gap += XRES(5);
		gEngfuncs.Con_Printf( "ancho %i\n", _gap );*/
...
CParanoiaTextPanel::CParanoiaTextPanel(char* filename) : Panel(0, 0, ScreenWidth, ScreenHeight)
{
	...
	if ( strstr(filename, "scripts/OS") != NULL )
	bCreatedByOS = true;
...
//	SchemeHandle_t hTextScheme = pSchemes->getSchemeHandle( "Title Font" );
	...
	SchemeHandle_t hTextScheme2 = pSchemes->getSchemeHandle( "LCDDot" );
	Font *pLCDDot = pSchemes->getFont( hTextScheme2 );
...
//	panel->setBorder(new LineBorder);
	...
	if( bCreatedByOS )
	{
		...
		panel->setBgColor(200, 200, 200, 0);
	}
...
/*	CCheckButton2* pSwitch = new CCheckButton2();
	...
	if(!strcmp(tagName, "TITLE"))
	{
		...
		if( bCreatedByOS )
		pTitle = new Label("", 0, 0, 0, 0);
		else
		pTitle = new Label("", XRES(10), upperBound, panel->getWide() - XRES(20), pDefaultFont->getTall()+YRES(8));
		...
		if( bCreatedByOS )
		{
 			pTitle->setFont(pLCDDot);
			pTitle->setPaintBackgroundEnabled(false);
			pTitle->setContentFitted(true);

			pTitle->setFgColor(255, 255, 255, 0.0);
	...
	while(1)
	{
		...
		if(!strcmp(tagName, "TEXT"))
		{
			...
			if (bgColorSet)
			{
				...
				if( bCreatedByOS )
				{
					panel->setSize(m_pScrollPanel->getWide(),m_pScrollPanel->getTall()+20);

					panel->setPos(ScreenWidth/2- m_pScrollPanel->getWide()/2,ScreenHeight/2-m_pScrollPanel->getTall()/2);
				
					m_pScrollPanel->setPos(0,20);

						
					button->setPos( panel->getWide() - 20, 0 );
				}
