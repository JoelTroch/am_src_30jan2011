...
void CItem::Spawn( void )
{
	...
	SetTouch(ItemTouch);
/*
	m_pBoxSprite = CSprite::SpriteCreate( "sprites/usable.spr", pev->origin, FALSE );				
			
	m_pBoxSprite->SetTransparency( kRenderGlow, 255, 255, 255, 255, kRenderFxNone );
	m_pBoxSprite->SetScale( 0.5 );
	m_pBoxSprite->SetAttachment( edict(), 0 );
	m_pBoxSprite->pev->frame = 0;
	m_pBoxSprite->pev->framerate = 0;
*/
...
void CItem::ItemTouch( CBaseEntity *pOther )
{
	...
	if (MyTouch( pPlayer ))
	{
		...
		{/*
			if ( m_pBoxSprite )
			{	
				ALERT (at_console, "SPRITE REMOVED\n");
				m_pBoxSprite = NULL;
			}*/
...
//		pPlayer->GiveNamedItem( "weapon_flash" );//fix

		//pPlayer->GiveNamedItem( "item_longjump" );//fix
...
CItem::Spawn( ); //I think this declares the spawn function of the parachute as a public item spawn function
	...
	void Precache( void ) //The precache function we called in spawn
	{
		...
		PRECACHE_MODEL("models/parachute_open.mdl");//moved to items.cpp
	}
...
class CItemNvg : public CItem //the nvg item class
{
void Spawn( void ) //the spawn function
{
Precache( ); //calls the precache function
SET_MODEL(ENT(pev), "models/w_battery.mdl"); //sets the model lying in the world to the longjump model. I think it looks similar to a parachute. You can also use the TFC backpack model for example
CItem::Spawn( ); //I think this declares the spawn function of the parachute as a public item spawn function
}
	void Precache( void ) //The precache function we called in spawn
	{
		PRECACHE_MODEL ("models/w_battery.mdl"); //this just precaches the model, otherwise you would get an error that it''s not precached. You can''t use unprecached models
		PRECACHE_SOUND( "items/nvg_on.wav" );
		PRECACHE_SOUND( "items/nvg_off.wav" );
	}
	BOOL MyTouch( CBasePlayer *pPlayer ) //MyTouch (if we pick the parachute up...)
	{
		if ( pPlayer->m_fNvg ) //if we have
		{
			return FALSE; //dont let us pick it up again
		}
		...
		if ( ( pPlayer->pev->weapons & (1<<WEAPON_SUIT) ) )//FIX
		{
			pPlayer->m_fNvg = TRUE; //...tell halflife we have the item
			
			// ... and tell them that we now have an NVG
			EMIT_SOUND( pPlayer->edict(), CHAN_ITEM, "items/gunpickup2.wav", 1, ATTN_NORM );
			ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "#PickUp_NVG"); //digamos al cliente
...
LINK_ENTITY_TO_CLASS( item_nvg, CItemNvg ); //makes an entity from the class we just wrote
