...
void CUsas::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > USAS_DEFAULT_GIVE) && (pev->armorvalue < 0))
		pev->armorvalue = USAS_DEFAULT_GIVE;
...
void CUsas::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/USAS/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/USAS/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/USAS/w_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( USAS_DEPLOY, 1, 0 );	
		return TRUE;
	}
	else	
	return DefaultDeploy( "models/weapons/USAS/v_USAS.mdl", "models/weapons/USAS/p_USAS.mdl", USAS_DEPLOY, "lr300", UseDecrement(), pev->body );
}
...
//	vecDir = m_pPlayer->FireBulletsPlayer( 6, vecSrc, vecAiming, VECTOR_CONE_10DEGREES, 2048, BULLET_PLAYER_USAS, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
	vecDir = m_pPlayer->FireBulletsPlayer( 6, vecSrc, vecAiming, VECTOR_CONE_10DEGREES, 2048, BULLET_PLAYER_EXPLOSIVE, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
	...
	m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 0.15; //() + 0.75;
