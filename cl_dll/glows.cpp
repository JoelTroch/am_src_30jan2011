...
{
//	if (!gl.IsGLAllowed() || v_glows == 0)
	if (!gl.IsGLAllowed() || (CVAR_GET_FLOAT("gl_glows") == 0) )
		return 1;

	if (ent->curstate.rendermode == kRenderGlow && ent->curstate.renderfx == kRenderFxNoDissipation)
	{
		if (numglows < CVAR_GET_FLOAT("gl_glows_max") )//MAX_GLOWS
		{
...
void DrawGlows()
{
	...
	if (gl.glIsEnabled(GL_DEPTH_TEST)) 
	depthtestenabled = 1;
...
void InitGlows()
{
//	v_glows = CVAR_GET_FLOAT("gl_glows");
