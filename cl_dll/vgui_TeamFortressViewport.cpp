...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	// VGUI MENUS
//PERFORMANCE:	we don't need these menus on SP
	CreateTeamMenu();
	CreateClassMenu();
...
//	if (m_pParanoiaText && m_pParanoiaText->isVisible()) //USED FOR ENABLE MOUSE IN TEXT WINDOWS

	if (m_pKeypad && m_pKeypad->isVisible())
bKeypadVisible = TRUE;
	else
bKeypadVisible = FALSE;

	gEngfuncs.Con_Printf("bKeypadVisible  %i \n", bKeypadVisible);

	if ( m_pSpectatorPanel->m_menuVisible || m_pCurrentMenu || m_pTeamMenu->isVisible() || m_pServerBrowser->isVisible() || GetClientVoiceMgr()->IsInSquelchMode() || m_pParanoiaText && m_pParanoiaText->isVisible() || m_pWeaponMenu && m_pWeaponMenu->isVisible()  || m_pKeypad && m_pKeypad->isVisible() || m_pOS && m_pOS->isVisible()  )
	{
...
void TeamFortressViewport::paintBackground()
{
	...
	// ======================================================
	/*
	if ( m_pHudMainPanel->isVisible() )
	{
		m_pHudMainPanel->Update();
		m_pHudMainPanel->Update_Timer();
	}*/

	// On doit le verifier а chaque frame
	
//	if ( CVAR_GET_FLOAT( "hud_vgui" ) != 0 )
//	m_pHudMainPanel->setVisible( true );
...
/*	// On doit le verifier а chaque frame
	if (	( gHUD.m_iHideHUDDisplay & ( HIDEHUD_WEAPONS | HIDEHUD_ALL ) ) || 
			( gEngfuncs.IsSpectateOnly() != 0 ) || 
			( g_iUser1 ) ||
			( g_iTeamNumber <= 0 ) ||
			( gHUD.m_Health.m_iHealth <= 0 ) )
...
int	TeamFortressViewport::KeyInput( int down, int keynum, const char *pszCurrentBinding )
{
	...
	//added by sys - MUST BE REMOVED CAUSE IT SUCKS
	...
	if (m_pKeypad && m_pKeypad->isVisible())
		return m_pKeypad->KeyInput(down, keynum, pszCurrentBinding);

	if (m_pOS && m_pOS->isVisible())
		return m_pOS->KeyInput(down, keynum, pszCurrentBinding);

	// buz
	if (down && m_pTabPanel && (keynum == K_TAB))
...
int TeamFortressViewport::MsgFunc_Keypad(const char *pszName, int iSize, void *pbuf )
{
	...
	m_pKeypad = new CKeypad();
	m_pKeypad->setParent(this);
	

	if (m_pKeypad)
	{
		sprintf(m_pKeypad->szCode, "%s", READ_STRING());
	
		sprintf(m_pKeypad->szFrameLabel, "%s",READ_STRING());

		m_pKeypad->iHidePassword = READ_LONG();
		m_pKeypad->iShowCables = READ_LONG();
	}

	m_pKeypad->Init();
	m_pKeypad->setVisible(true);
	UpdateCursorState();

	return 1;
}

int TeamFortressViewport::MsgFunc_OS(const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
	...
	if (m_pOS)
	{
		sprintf(m_pOS->szPCName, "%s", READ_STRING());

		m_pOS->bLogged	= READ_BYTE();

		sprintf(m_pOS->szLogUser, "%s", READ_STRING());
		sprintf(m_pOS->szLogPassword, "%s", READ_STRING());	
		
		m_pOS->iFinalCode	= READ_LONG();
	}

	if( m_pOS->bLogged == false )
	m_pOS->fDelay = 1000.0 /*gEngfuncs.pfnRandomFloat( 500.0, 2000.0 )*/;
	else
	m_pOS->fDelay = 0.0;
...
int TeamFortressViewport::MsgFunc_ShowTextWindow(const char *pszName, int iSize, void *pbuf )
{
	...
	//CRASHES AFTER SAVE-LOADING

	if (m_pParanoiaText)
	{
	//	if (strcmp(m_pParanoiaText->m_loadedFileName, str))//causes crashes after save/loading
	//	{
			removeChild(m_pParanoiaText);
			m_pParanoiaText = NULL;//added
