...
//	_snprintf(m_pSystem->sParticleNewSysForTraces, MAX_PARTICLE_PATH, "");
...
//		gEngfuncs.Con_Printf("Bad Mapped Particle definition file specified %s\n", m_sParticleFile);
	...
	//sys test
/*
		// how traces are handled
		else if(!stricmp(sSetting, "create_trace")) 
		{
			m_pSystem->bCreateTraces = true;
			_snprintf(m_pSystem->sParticleNewSysForTraces, MAX_PARTICLE_PATH, "%s", sValue);
		}*/
	//sys test
	//sys test
	//sys test
