...
void CCrowbar::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/knife/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/knife/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/knife/w_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( KNIFE_DEPLOY, 1, 0 );	
		return TRUE;
	}
	else
	return DefaultDeploy( "models/weapons/knife/v_knife.mdl", "models/weapons/knife/p_knife.mdl", KNIFE_DEPLOY, "psknife" ); //"crowbar"
}
	...
	else
	{
		...
		case 0:
			SendWeaponAnim( KNIFE_SLASH2 ); break;
							m_pPlayer->ViewPunch(5.5, -5.5, 0);

		case 1:
			SendWeaponAnim( KNIFE_SLASH1 ); break;
										m_pPlayer->ViewPunch(5.5, -2.5, 0);
		...
		if (pEntity)
		{
			if ( pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE )
			{
				...
				m_pPlayer->ViewPunch(RANDOM_LONG(-2,2), RANDOM_LONG(-3,3), RANDOM_LONG(-2,2));//abajo-arriba -derecha-izquierda? -circular izquierda-derecha
	...
	else
	{
		...
		if (pEntity)
		{
			if ( pEntity->Classify() != CLASS_NONE && pEntity->Classify() != CLASS_MACHINE )
			{
				...
				m_pPlayer->ViewPunch(RANDOM_LONG(-6,0), RANDOM_LONG(-3,3), RANDOM_LONG(-2,2));//abajo-arriba -derecha-izquierda? -circular izquierda-derecha
		...
		if (fHitWorld)
		{
			...
			m_pPlayer->ViewPunch(RANDOM_LONG(-5,0), RANDOM_LONG(-3,3), RANDOM_LONG(-2,2));//abajo-arriba -derecha-izquierda? -circular izquierda-derecha
