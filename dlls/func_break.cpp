...
void CBreakable::Precache( void )
{
	...
	pMemFile = LOAD_FILE_FOR_ME((char *)pGibName, &length);
	...
	if ( playerTouch )
	{
		...
		if ( (gpGlobals->time - m_soundTime) > 0.7 )
		{
			...
			if ( length > 0 && FBitSet(pev->flags,FL_ONGROUND) )
			{
				...
				CSoundEnt::InsertSound ( bits_SOUND_COMBAT, pev->origin, 666, 0.3 );//sys
	//			SetThink( StopSound );
	//			pev->nextthink = pev->ltime + 0.1;
			}
			else
...
class CPhysModel : public CBaseAnimating
{
public:
	void Spawn( void );
	void EXPORT ModelThink( void );
	
	int	Save( CSave &save ); 
	int Restore( CRestore &restore );
	static TYPEDESCRIPTION m_SaveData[];
	
	CBaseEntity	*pBrushModel;
};

LINK_ENTITY_TO_CLASS( phys_render_model, CPhysModel );

TYPEDESCRIPTION	CPhysModel::m_SaveData[] = 
{
	DEFINE_FIELD( CPhysModel, pBrushModel, FIELD_CLASSPTR )
};

IMPLEMENT_SAVERESTORE( CPhysModel, CBaseAnimating );

void CPhysModel :: Spawn(void)
{
	pBrushModel = Instance( pev->owner );

	SetThink(&CPhysModel::ModelThink);
	pev->nextthink = gpGlobals->time + 0.1;
	SetNextThink( 0.1 );
}

void CPhysModel :: ModelThink(void)
{
	pev->angles = pBrushModel->pev->angles;
	pev->origin = pBrushModel->pev->origin;
	pev->origin.z = pBrushModel->pev->origin.z - 14;//this determines the position???

//	pev->origin.z = pCar->pev->origin.z + 64;//this determines the position???
	
	pev->velocity = pBrushModel->pev->velocity;
	pev->avelocity = pBrushModel->pev->avelocity;
...
LINK_ENTITY_TO_CLASS( func_phys_model, CPhy );
...
//	pev->gravity = 0.1;
	...
	m_soundTime = 0;
/*
//	if ( FClassnameIs ( pev, "func_phys_model" ) )
//	{
//		const char *pModelName = (STRING(pev->model));

		//create the model render ent
		pModel = GetClassPtr( (CPhysModel *)NULL );
...
//		SET_MODEL(ENT(pModel->pev), (char *)pModelName);

		pModel->pev->origin = pev->origin;
		UTIL_SetOrigin(pModel->pev, pev->origin);

		pModel->pev->classname = MAKE_STRING("phys_render_model");
