...
void CGlock::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > GLOCK_DEFAULT_GIVE) && (pev->armorvalue < 0))
		pev->armorvalue = GLOCK_DEFAULT_GIVE;
...
void CGlock::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/1911/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/1911/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/1911/w_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( GLOCK_DEPLOY, 1, 0 );	
		return TRUE;
	}
	else
	return DefaultDeploy( "models/weapons/1911/v_9mmhandgun.mdl", "models/weapons/1911/p_9mmhandgun.mdl", GLOCK_DEPLOY, "p22", /*UseDecrement() ? 1 : 0*/ 0 );
}
...
void CGlock::PrimaryAttack( void )
{
    ...
    return;
   	GlockFire( 0.02, 0.15, TRUE ); //GlockFire( 0.01, 0.3, TRUE );
	return;
}
...
//		if (pOther->GiveAmmo( AMMO_GLOCKCLIP_GIVE, "9mm", _9MM_MAX_CARRY ) != -1)
		if (pOther->GiveAmmo( AMMO_GLOCKCLIP_GIVE, "ammo_45", _9MM_MAX_CARRY ) != -1)
		{
