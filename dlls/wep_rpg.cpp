...
//				pev->velocity = ( ((pev->velocity.Normalize() * 2000) * 25) / 100);
//				else
...
void CRpg::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/w_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( RPG_DRAW1, 1, 0 );	
		return TRUE;
	}
	else
	return DefaultDeploy( "models/v_rpg.mdl", "models/null.mdl", RPG_DRAW1, "rpg" );
}
