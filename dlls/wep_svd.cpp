...
// ### COD RECOIL END ###
		...
		{	
			...
			{	//  si esta en el suelo y agachado
				vecDir = m_pPlayer->FireBulletsThroughWalls( 1, vecSrc, vecAiming, VECTOR_CONE_PERFECT, 8192, BULLET_PLAYER_M82, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
			}
			else // si no es porq esta parado
			{																//perfe 3d
				vecDir = m_pPlayer->FireBulletsThroughWalls( 1, vecSrc, vecAiming, VECTOR_CONE_3DEGREES, 8192, BULLET_PLAYER_M82, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
			}
		}
		else // si no esta en el suelo es porque esta en el aire
		{
			...
			{// y si esta agachado en el aire gana un punto
				vecDir = m_pPlayer->FireBulletsThroughWalls( 1, vecSrc, vecAiming, VECTOR_CONE_20DEGREES, 2048, BULLET_PLAYER_M82, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
			}						// 14 puntos igual
			else
...
// ### COD RECOIL END ###
  int flags;
    ...
    m_flNextPrimaryAttack = UTIL_WeaponTimeBase() + 1.9;  // 1.15;  //SVD_FIRE_DELAY 1.95
    m_flTimeWeaponIdle    = UTIL_WeaponTimeBase() + UTIL_SharedRandomFloat(m_pPlayer->random_seed, 21.53 + 21, 21.53 + 20); //20 1
	
	m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 1.9; //zoom
...
m_fInZoom = 0;
	...
	iResult = DefaultReload( 5, SVD_RELOAD, 2.7 );
