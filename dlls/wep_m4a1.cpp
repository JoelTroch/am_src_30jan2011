...
void CM16::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > M16_DEFAULT_AMMO) && (pev->armorvalue < 0))
		pev->armorvalue = M16_DEFAULT_AMMO;
...
void CM16::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/m4a1/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/m4a1/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/m4a1/w_%s.mdl", pkvd->szValue);
...
//			m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( M16_SIL_DEPLOY, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy("models/weapons/m4a1/v_m4a1.mdl", "models/weapons/m4a1/p_m4a1_2.mdl", M16_SIL_DEPLOY, "sniperscope", 2.50f );
	}
    else
	{*/
		if (!FStringNull (v_model) )
...
//			m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( M16_DEPLOY, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy("models/weapons/m4a1/v_m4a1.mdl", "models/weapons/m4a1/p_m4a1.mdl", M16_DEPLOY, "famas", 2.50f );
//	}
...
/*
  ...
  //  PLAYBACK_EVENT_FULL( flags, m_pPlayer->edict(), m_usFireM161, 0.0, (float *)&g_vecZero, (float *)&g_vecZero, vecDir.x, vecDir.y, 0, 0, ( m_iClip == 0 ) ? 0:0, 0 );
	...
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/m4a1/m4a1_Fire-1.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0,0xF));

	if (RANDOM_LONG(0,1))
	SendWeaponAnim( M16_FIRE1 );
	else
	SendWeaponAnim( M16_FIRE2 );
...
/*	if(m_fDefaultAnim != 0)
	DefaultReload( 30, M16_SIL_RELOAD_NMC_VACIO, 2.7 );
	else*/
	if (m_iClip == 0)
	DefaultReload( 30, M16_RELOAD1, 3.38 );
	else
	DefaultReload( 30, M16_RELOAD_VACIO, 3.38 );
