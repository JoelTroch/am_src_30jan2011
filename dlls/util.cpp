...
void UTIL_ParticleEmiterPreset( const Vector &vecOrigin, const unsigned &iPreset )
{
...
void UTIL_ParticleEmiterFromFile( const Vector &vecOrigin, const char * file )
{
	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);//enviar a todos... qué importa??
		...
		WRITE_COORD(vecOrigin.y);
		WRITE_COORD(vecOrigin.z);
		WRITE_COORD(0);//not used angles
		WRITE_COORD(0);
		WRITE_COORD(0);
		WRITE_SHORT(0);
		WRITE_STRING(STRING(file));
	MESSAGE_END();
}
...
void UTIL_BloodDrips( const Vector &origin, const Vector &direction, int color, int amount )
{
	...
	if ( (CVAR_GET_FLOAT("cl_blood_type" ) != 0) || CVAR_GET_FLOAT("r_particles" ) == 0 )
	{		
	/*	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, origin );
...
void UTIL_DecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	switch (decalNumber)
	{
		...
		case DECAL_HOLE_CONC1: 
		case DECAL_HOLE_CONC2: 
		case DECAL_HOLE_CONC3: 
			UTIL_CustomDecal( pTrace, "wallshot", 0 );
		break;
		
		case DECAL_HOLE_METAL1: 
		case DECAL_HOLE_METAL2: 
		case DECAL_HOLE_METAL3: 
			UTIL_CustomDecal( pTrace, "metalshot", 0 );
		break;
			
		case DECAL_HOLE_WOOD1: 
		case DECAL_HOLE_WOOD2: 
		case DECAL_HOLE_WOOD3: 
			UTIL_CustomDecal( pTrace, "woodshot", 0 );
		break;
	}
...
/*	Vector vEndpos = origin;
	...
	for( int i = 0; i < 10; i++ )
	{
		...
		iSpread = RANDOM_LONG(1,20);
		vEndpos = vEndpos + direction * iSpread;
		...
		pBeam[i]->PointsInit( origin, vEndpos );
		pBeam[i]->SetNoise( 1 );
...
BOOL UTIL_IsTextureSky( TraceResult *tr, Vector vecSrc, Vector vecEnd )
{
