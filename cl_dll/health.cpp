...
DECLARE_MESSAGE( m_Health, Armor )
DECLARE_MESSAGE( m_Health, SlowMotion )
...
int CHudHealth:: MsgFunc_Armor( const char *pszName,  int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
...
int CHudHealth:: MsgFunc_SlowMotion(const char *pszName,  int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
...
//	gViewPort->m_pHud2->UpdateSlowMotion(x); // buz
...
/*		if ( CVAR_GET_FLOAT( "hud_vgui" ) == 0 )
...
#else
		//NEW CODE
		int h_r = 0, h_g = 0, h_b = 0;

		if( CVAR_GET_FLOAT( "light_amount" ) <= 10 )
		{
			h_r = CVAR_GET_FLOAT( "light_amount" );
			h_g = CVAR_GET_FLOAT( "light_amount" );
			h_b = 255;
		}
		else
		{
			h_r = CVAR_GET_FLOAT( "trace_distance" );
			h_g = CVAR_GET_FLOAT( "trace_distance" );
			h_b = CVAR_GET_FLOAT( "trace_distance" );
		}
		...
		if ( CVAR_GET_FLOAT( "hud_newcross" ) != 0 )
		{
			...
			if ( CVAR_GET_FLOAT( "hud_newcross_size" ) == 1 )//small | mayor
			{
        		...
        		FillRGBA(x, y, iWidth, iHeight, h_r, h_g, h_b, 255);//test
