...
/*		if (pev->message)				
		...
		if (pev->message)
		EMIT_SOUND_SUIT( pPlayer->edict(), STRING(pev->message));
	...
	else if ( FClassnameIs ( pev, "item_secret" ) )
	{
...
{	public:		void Spawn( void ){ GenericCineEntitySpawn( "models/npccamera_test.mdl", Vector(-16,-16,-16), Vector(16,16,16)); } };
LINK_ENTITY_TO_CLASS( npc_actor, CCine01 );
...
{	public:		void Spawn( void ){ GenericCineEntitySpawn( "models/terrorist.mdl", Vector(-16,-16,-16), Vector(16,16,16)); } };
...
void CCineEnt :: Spawn( void )
{
	...
	if ( pPlayer )
	{
		...
		GenericCyclerSpawn( "models/terrorist.mdl", Vector(-16,-16,-16), Vector(16,16,16));
