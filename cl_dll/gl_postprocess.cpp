...
{
	float grayscale = CVAR_GET_FLOAT("gl_grayscale");
...
void DrawQuadForPostProcess(int width, int height, int ofsX = 0, int ofsY = 0)
{
...
{
	if (!gl.IsGLAllowed() || (CVAR_GET_FLOAT("gl_posteffects") == 0) )
		return;
	...
	//	gl.glCopyTexImage2D(GL_TEXTURE_RECTANGLE_NV, 0, GL_RGB, 0, 0, ScreenWidth, ScreenHeight, 0);
		...
		gl.glBegin(GL_QUADS);
		DrawQuadForPostProcess(ScreenWidth, ScreenHeight);
		gl.glEnd();
...
//	v_posteffects = CVAR_GET_FLOAT("gl_posteffects");
//	v_grayscale = CVAR_GET_FLOAT("gl_grayscale");
}