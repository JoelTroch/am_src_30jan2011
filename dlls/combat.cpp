...
#include "customentity.h"//BEAM_SOLID
...
//			deathActivity = ACT_DIE_BACKSHOT;
	...
	if ( deathActivity == ACT_DIEBACKWARD )
	{
		...
		{
			//sys add!
			if ( FClassnameIs( pev, "npc_combine_metrocop" ) 
				|| FClassnameIs( pev, "npc_combine" )
				|| FClassnameIs( pev, "npc_combine_supersoldier" )
				|| FClassnameIs( pev, "npc_combine_random" )
				|| FClassnameIs( pev, "monster_human_ak" )
			//	|| FClassnameIs( pev, "monster_terrorist" )
//				|| FClassnameIs( pev, "monster_human_grunt" )
				|| FClassnameIs( pev, "monster_human_grunt_opfor" )
				)	
			{
		...
		if ( phys_flyattack.value != 0  ) //no es cero esta _Activado_
		{
			...
			//	{
					...
					//	pev->velocity = pev->velocity * RANDOM_FLOAT( 150, 250 );//200 400 too much!
						pev->velocity = pev->velocity * RANDOM_FLOAT( 50, 150 );//200 400 too much!
					}
			//	}
			}
			else
...
*/
		...
		else
		{
			...
			else//simple
			{
				...
				if ( m_pPlayer->b_PlayerCanHideInShadows )
				{
				//	UTIL_ShowMessageAll( STRING(ALLOC_STRING("SHADOW_STYLE")));
					DBSetExperiencePoints( 3 );
				}
				else
				{
				//	UTIL_ShowMessageAll( STRING(ALLOC_STRING("SIMPLE_STYLE")));
					DBSetExperiencePoints( 1 );		
				}
...
//*******
	...
	if ( (pev->flags & FL_MONSTER) && !FNullEnt(pevAttacker) )
	{
		...
		{
		//	UTIL_ShowMessageAll( STRING(ALLOC_STRING("Hit")));
...
*//*
			...
			{
				case 0:EMIT_SOUND(ENT(pev), CHAN_STATIC, "player/damage/headshot2.wav", 0.9, ATTN_NORM); break;
...
{
	//do not use FireBulletsThroughWalls
	//return here
...
*/
				...
				//	{
						MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
							...
							{	
								if ( RANDOM_LONG(0,99) < 20 )// 39.6 % of chance
...
CBullet *CBullet::BulletCreate( void )//NOT CALLED
{
...
/*
	...
	m_pBeam = CBeam::BeamCreate( "sprites/lgtning.spr", 10 );

	m_pBeam->PointEntInit( vecSrc, entindex( ) );
...
/*
	//Ban-aid
	// If for some reason the bullet doesn't have any vel, let's add a default one
	if( pShoot->pev->velocity == g_vecZero )
	{
		pShoot->pev->velocity = pShoot->pev->velocity + (gpGlobals->v_forward * 100);
	}*/
}

void CBullet :: Touch ( CBaseEntity *pOther )
{
	if ( FClassnameIs(pev, "bullet_pl") || FClassnameIs(pev, "bullet_pl") || FClassnameIs(pev, "bullet_pl") && (FClassnameIs(pOther->pev, "player")))//it's a friendly bullet and i'm hit whit a player?
		return;//do nothing!

	//dont hit itself
/*	if (FClassnameIs(pev, "bullet_pl") && (FClassnameIs(pOther->pev, "player")))//it's a friendly bullet and i'm hit whit a player?
	{
		return;//do nothing!
	}
	else//i'm a normal bullet*/
	{
		/*
		SetTouch( NULL );
		SetThink( NULL );
...
//			pOther->TraceAttack(pevOwner, gSkillData.plrDmgCrossbowMonster, pev->velocity.Normalize(), &tr, DMG_BULLET | DMG_NEVERGIB ); 
		...
		if (!FClassnameIs(pOther->pev, "bullet"))//i'm hitting whit my self? EDIT: I guess this isn't neccesary
		{
				//no! so let's make damage!
			TraceResult tr;
			int		iPitch;
			...
			if ( !pOther->pev->takedamage )
			{
				...
				Vector vecSrc, vecEnd = pev->origin;
				...
				if ( pOther->IsBSPModel() )
				{
					...
					if (pOther && pOther->Classify() != CLASS_NONE && pOther->Classify() != CLASS_MACHINE)
						// hit body
						chTextureType = CHAR_TEX_FLESH;
					else
					{
						...
						if (pOther)
							pTextureName = TRACE_TEXTURE( ENT(pOther->pev), rgfl1, rgfl2 );
						else
							pTextureName = TRACE_TEXTURE( ENT(0), rgfl1, rgfl2 );
					...
					if ( FClassnameIs(pOther->pev, "func_collision"))
					{
						b_CanMakeParticles = FALSE;
					}
...
//								UTIL_CustomDecal( &tr, "shot" );
			...
			{
			//	if (FClassnameIs(pev, "bullet_pl") )//bullets shoot by the player aren't made for... hurt the player
			//	{
					TraceResult tr = UTIL_GetGlobalTrace( );
					entvars_t	*pevOwner;
...
*/
	...
	//	pev->velocity = pev->velocity + (gpGlobals->v_forward * 100);//force to collide 'em
	}
//	else
//	{
...
//	}
	...
	if ( CVAR_GET_FLOAT("slowmo" ) == 1 )			
	{
		MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
				...
				WRITE_COORD( 0 );
				WRITE_COORD( 0 );
				WRITE_SHORT(iDefaultSlowMoEffect);
			MESSAGE_END();
	}
...
/*	if ( CVAR_GET_FLOAT("r_shadows" ) != 0 )
	{
		CBaseEntity *pLight = CBaseEntity::Create( "env_light", v, g_vecZero, NULL );
		pLight->pev->renderamt = 25;
		pLight->pev->rendercolor.x = 255;
		pLight->pev->rendercolor.y = 255;
		pLight->pev->rendercolor.z = 128;
		pLight->Use( this, this, USE_ON, 1 );
		pLight->SetThink( SUB_Remove );
		pLight->pev->nextthink = gpGlobals->time + 0.03;
	}
*/
...
//			CBeam *pBeam[10];
			...
			{			
				...
				/*		Vector angles = UTIL_VecToAngles( tr.vecPlaneNormal );
						Create( "sfx_sparks", tr.vecEndPos + tr.vecPlaneNormal * RANDOM_LONG(0.5,0.1), angles, NULL );
					*/
					/*	for( int i = 0; i < 10; i++ )
						{
							pBeam[i] = CBeam::BeamCreate( "sprites/rope.spr", RANDOM_LONG(1,5) );
								
							iSpread = RANDOM_LONG(1,15);
							vEndpos = vEndpos + tr.vecPlaneNormal * iSpread;
							vEndpos = vEndpos + gpGlobals->v_forward * RANDOM_LONG(-iSpread,iSpread) + gpGlobals->v_right * RANDOM_LONG(-iSpread,iSpread) + gpGlobals->v_up * RANDOM_LONG(-iSpread,iSpread);

							pBeam[i]->PointsInit( tr.vecEndPos, vEndpos );
							pBeam[i]->SetNoise( 1 );
							pBeam[i]->SetFlags( BEAM_FSHADEOUT );
							pBeam[i]->SetColor( 255, 255, 255 );
							pBeam[i]->SetThink( SUB_Remove );
							pBeam[i]->pev->nextthink = gpGlobals->time + 0.1;
						}*/
...
Vector CBaseEntity::FireBulletsPlayer ( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
	...
	if ( ( pPlayer->pev->weapons & (1<<WEAPON_SUIT) ) && (pPlayer->m_pActiveItem->m_iId != NULL) )//FIX
	{
		...
		{
			case  WEAPON_SHOTGUN:					vecSpread = VECTOR_CONE_10DEGREES; break;
			case  WEAPON_USAS:						vecSpread = VECTOR_CONE_10DEGREES; break;
			case  WEAPON_SHOTGUN_LESSLETHAL:		vecSpread = VECTOR_CONE_10DEGREES; break;
			case  WEAPON_SNIPER:	
...
/*	if ( CVAR_GET_FLOAT( "r_shadows" ) != 0 )
	{
		CBaseEntity *pLight = CBaseEntity::Create( "shadow_light", pev->origin, g_vecZero, NULL );
		pLight->pev->renderamt = 10;
		pLight->pev->rendercolor.x = 255;
		pLight->pev->rendercolor.y = 255;
		pLight->pev->rendercolor.z = 255;
		pLight->Use( this, this, USE_ON, 1 );
		pLight->SetThink( SUB_Remove );
		pLight->pev->nextthink = gpGlobals->time + 0.03;
	}*/
	...
	for ( ULONG iShot = 1; iShot <= cShots; iShot++ )
	{
		...
		if (pPlayer->m_fSlowMotionOn)
		{
			#define BULLET_SPEED	100//2000
			#define BOLT_AIR_VELOCITY	500
			#define BOLT_WATER_VELOCITY	250

			if( cShots > 1 )//I bet this is a shotgun ^^
			{
				Vector anglesAim = pPlayer->pev->v_angle + pPlayer->pev->punchangle;
				UTIL_MakeVectors( anglesAim );
				
				anglesAim.x		= -anglesAim.x;
				Vector vecSrc	 = pPlayer->GetGunPosition( ) + gpGlobals->v_forward * 16 - gpGlobals->v_up * 2 + gpGlobals->v_right * 4;
				Vector vecDir	 = gpGlobals->v_forward;

				CBaseEntity *pBolt = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt->pev->origin = vecSrc;
				pBolt->pev->angles = anglesAim;
				pBolt->pev->owner = pPlayer->edict();
				pBolt->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt->pev->solid = SOLID_BBOX;
				...
				CBaseEntity *pBolt1 = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt1->pev->origin = vecSrc;
				pBolt1->pev->angles = anglesAim;
				pBolt1->pev->owner = pPlayer->edict();
				pBolt1->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt1->pev->solid = SOLID_BBOX;

				if (pPlayer->pev->waterlevel == 3)
				{
					pBolt1->pev->velocity = vecDir * BOLT_WATER_VELOCITY;
					pBolt1->pev->speed = BOLT_WATER_VELOCITY;
				}
				else
				{
					pBolt1->pev->velocity = vecDir * BOLT_AIR_VELOCITY;
					pBolt1->pev->speed = BOLT_AIR_VELOCITY;
				}
				pBolt1->pev->avelocity.z = 10;


				CBaseEntity *pBolt2 = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt2->pev->origin = vecSrc;
				pBolt2->pev->angles = anglesAim;
				pBolt2->pev->owner = pPlayer->edict();
			//	pBolt2->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt2->pev->solid = SOLID_BBOX;

				if (pPlayer->pev->waterlevel == 3)
				{
					pBolt2->pev->velocity = vecDir * BOLT_WATER_VELOCITY;
					pBolt2->pev->speed = BOLT_WATER_VELOCITY;
				}
				else
				{
					pBolt2->pev->velocity = vecDir * BOLT_AIR_VELOCITY;
					pBolt2->pev->speed = BOLT_AIR_VELOCITY;
				}
				pBolt2->pev->avelocity.z = 10;


				CBaseEntity *pBolt3 = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt3->pev->origin = vecSrc;
				pBolt3->pev->angles = anglesAim;
				pBolt3->pev->owner = pPlayer->edict();
			//	pBolt3->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt3->pev->solid = SOLID_BBOX;

				if (pPlayer->pev->waterlevel == 3)
				{
					pBolt3->pev->velocity = vecDir * BOLT_WATER_VELOCITY;
					pBolt3->pev->speed = BOLT_WATER_VELOCITY;
				}
				else
				{
					pBolt3->pev->velocity = vecDir * BOLT_AIR_VELOCITY;
					pBolt3->pev->speed = BOLT_AIR_VELOCITY;
				}
				pBolt3->pev->avelocity.z = 10;


				CBaseEntity *pBolt4 = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt4->pev->origin = vecSrc;
				pBolt4->pev->angles = anglesAim;
				pBolt4->pev->owner = pPlayer->edict();
			//	pBolt4->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt4->pev->solid = SOLID_BBOX;

				if (pPlayer->pev->waterlevel == 3)
				{
					pBolt4->pev->velocity = vecDir * BOLT_WATER_VELOCITY;
					pBolt4->pev->speed = BOLT_WATER_VELOCITY;
				}
				else
				{
					pBolt4->pev->velocity = vecDir * BOLT_AIR_VELOCITY;
					pBolt4->pev->speed = BOLT_AIR_VELOCITY;
				}
				pBolt4->pev->avelocity.z = 10;


				CBaseEntity *pBolt5 = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
				pBolt5->pev->origin = vecSrc;
				pBolt5->pev->angles = anglesAim;
				pBolt5->pev->owner = pPlayer->edict();
			//	pBolt5->pev->classname		= MAKE_STRING("bullet_pl");
			//	pBolt5->pev->solid = SOLID_BBOX;

				if (pPlayer->pev->waterlevel == 3)
				{
					pBolt5->pev->velocity = vecDir * BOLT_WATER_VELOCITY;
					pBolt5->pev->speed = BOLT_WATER_VELOCITY;
				}
				else
				{
					pBolt5->pev->velocity = vecDir * BOLT_AIR_VELOCITY;
					pBolt5->pev->speed = BOLT_AIR_VELOCITY;
				}
				pBolt5->pev->avelocity.z = 10;
			...
			CBaseEntity *pBolt;
			pBolt = CBaseEntity::Create( "bullet", vecSrc, pPlayer->pev->v_angle, pPlayer->edict() );
			...
			DynamicPunch(SPREAD_DEFAULT);

			return Vector( x * vecSpread.x, y * vecSpread.y, 0.0 );
		}
		else//this else is kinda stupid*/
		{
...
//		if ( UTIL_IsTextureSky( &tr, vecSrc, vecEnd ) )
//		BOOL b_CanMakeParticles = FALSE;
		...
		if ( pEntity->IsBSPModel() )
		{
			...
			if ( pEntity && pEntity->pev->rendermode != 0 )
						b_CanMakeParticles = FALSE;
...
/*
				...
				/*	CBasePlayer *m_pPlayer = GetClassPtr((CBasePlayer *)pev);
					...
					if ( (m_pPlayer->m_pActiveItem->m_iId != NULL) )//FIX
					{
						if ( m_pPlayer->m_pActiveItem->m_iId == WEAPON_SHOTGUN_LESSLETHAL )
						{
							switch ( RANDOM_LONG(0,2) )
							{
								case 0: UTIL_EmitAmbientSound(ENT(0), tr.vecEndPos, "weapons/bulletimpact/concrete1.wav", flVolume, ATTN_NORM, 0, 100); break;
...
//						UTIL_CustomDecal( &tr, "shot" );			
/*
						CSprite *pSprite = CSprite::SpriteCreate( "sprites/bloodspot.spr", tr.vecEndPos + tr.vecPlaneNormal * 8, FALSE );		
						pSprite->SetTransparency( kRenderTransAlpha, 255, 255, 255, 111, kRenderFxNoDissipation );
						pSprite->SetScale( 1.0 );

					//	pSprite->pev->angles = tr.vecPlaneNormal;

											
						pSprite->pev->angles = angles;
							*/
					}			
...
//////////////////////////////////////////////////////////////////
			...
			default:
				...
				case BULLET_PLAYER_M82:
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgM82, vecDir, &tr, DMG_BULLET);
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				break;

				case BULLET_PLAYER_SCOUT:
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgScout, vecDir, &tr, DMG_BULLET);
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				break;

				case BULLET_PLAYER_FAMAS:
				pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgFamas, vecDir, &tr, DMG_BULLET);
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				break;
				...
				case BULLET_PLAYER_RUBBER:
				pEntity->TraceAttack(pevAttacker,  1/*gSkillData.plrDmgBuckshot*/, vecDir, &tr, DMG_PELLETS ); 
				TEXTURETYPE_PlaySound(&tr, vecSrc, vecEnd, iBulletType);
				break;
...
/*
void CGib :: GrenadeGibs( entvars_t *pevVictim, int cGibs, int human )
{
	int cSplat;
	...
	for ( cSplat = 0 ; cSplat < cGibs ; cSplat++ )
	{
		...
		pGib->Spawn( "sprites/materials/grenade_gibs.spr" );
//		pGib->Spawn( "models/weapons/grenade_gibs.mdl" );
		pGib->pev->body = RANDOM_LONG(0,ALIEN_GIB_COUNT-1);
		...
		if ( pevVictim )
		{
			...
			// make the gib fly away from the attack vector
		//	pGib->pev->velocity = g_vecAttackDir * -1;
			pGib->pev->velocity = g_vecAttackDir * 5;

			// mix in some noise
			pGib->pev->velocity.x += RANDOM_FLOAT ( -0.25, 0.25 );
		//	pGib->pev->velocity.y += RANDOM_FLOAT ( -0.25, 0.25 );
			pGib->pev->velocity.y += RANDOM_FLOAT ( -1, 1 );
			pGib->pev->velocity.z += RANDOM_FLOAT ( -0.25, 0.25 );
			...
			pGib->pev->avelocity.x = RANDOM_FLOAT ( 100, 200 );
		//	pGib->pev->avelocity.y = RANDOM_FLOAT ( 100, 300 );
			pGib->pev->avelocity.y = RANDOM_FLOAT ( 300, 500 );
...
void CBaseEntity::DynamicPunch(int iWeaponType)
{
	...
	if ( ( m_pPlayer->pev->weapons & (1<<WEAPON_SUIT) ) && (m_pPlayer->m_pActiveItem->m_iId != NULL) )//FIX
	{
		...
		{
			case  WEAPON_GLOCK:		fMySpread = 1.0; break;//1911
			case  WEAPON_PYTHON:	fMySpread = 4.0; break;
			case  WEAPON_MP5:		fMySpread = 0.7; break;
			case  WEAPON_SHOTGUN:	fMySpread = 4.0; break;//3.5
			case  WEAPON_USAS:		fMySpread = 3.0; break;
			case  WEAPON_AK74:		fMySpread = 1.0; break;
			case  WEAPON_M16:		fMySpread = 0.9; break;
			case  WEAPON_DEAGLE:	fMySpread = 2.0; break;
			case  WEAPON_SNIPER:	fMySpread = 3.0; break;
			case  WEAPON_FAMAS:		fMySpread = 0.8; break;
			case  WEAPON_M249:		fMySpread = 1.4; break;
			case  WEAPON_UZI:		fMySpread = 1.0; break;//p90
			case  WEAPON_SKORPION:		fMySpread = 1.5; break;
			case  WEAPON_BER92F:	fMySpread = 0.6; break;
			case  WEAPON_M4A2:		fMySpread = 1.0; break;
			case  WEAPON_GLOCK18:	fMySpread = 0.8; break;
			case  WEAPON_OICW:		fMySpread = 0.9; break;
			case  WEAPON_SHOTGUN_LESSLETHAL:		fMySpread = 2.0; break;
			case  WEAPON_SVD:		fMySpread = 10.0; break;
	...
	if ( CVAR_GET_FLOAT( "pl_using_silencer" ) == 0 )
	{
		MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
			WRITE_BYTE( TE_DLIGHT );
			WRITE_COORD( pev->origin.x ); // origin
	...
	m_pPlayer->ViewPunch(fSpreadP, RANDOM_FLOAT(-fSpreadY,fSpreadY), RANDOM_FLOAT(-fSpreadR,fSpreadR) );
