...
/*
	glDisable(GL_BLEND);
	glDepthMask(GL_FALSE); // re-enable modification of the depth buffer
	glDepthFunc(GL_LESS);	
*/
...
{
	// Highlander
	// Comment this out if you don't want to restrict 
	// the players to play with the hacked dll.
...
/*
	m_pCurrentEntity->curstate.gaitsequence = 10;

	if (m_pCurrentEntity->curstate.gaitsequence)
	{
		gEngfuncs.Con_Printf( "gaitsequence\n" );
		vec3_t orig_angles;
		m_pPlayerInfo = IEngineStudio.PlayerInfo( m_nPlayerIndex );

		VectorCopy( m_pCurrentEntity->angles, orig_angles );
	
		StudioProcessGaitForEnt( m_pCurrentEntity );
...
*/
	StudioSetUpTransform( 0 );//used above
	...
	if (flags & STUDIO_EVENTS)
	{
		...
		if ( m_pCurrentEntity->index > 0 )
		{
			...
			memcpy( ent->attachment, m_pCurrentEntity->attachment, sizeof( vec3_t ) * MAX_ATTACHMENTS );//4
		}
	}
...
/*		if ( m_pCvarShadowsDynamic->value <= 1 )
		{
			m_vLightmapAngle[0] = -lighting.plightvec[0];
			m_vLightmapAngle[1] = -lighting.plightvec[1];
			m_vLightmapAngle[2] = -lighting.plightvec[2];
		}
		else
		{
			m_vLightmapAngle[0] = 0.5;
			m_vLightmapAngle[1] = 1;
			m_vLightmapAngle[2] = 1.5;
		}*/
		// Highlander end
...
//CONPRINT("m_pCurrentEntity->curstate.body %i\n", m_pCurrentEntity->curstate.body );
...
void CStudioModelRenderer::StudioEstimateGait( entity_state_t *pplayer )
{
	...
	{
		VectorCopy( m_pCurrentEntity->curstate.velocity, est_velocity );
...
====================
*/
void CStudioModelRenderer::StudioEstimateGaitForEnt ( cl_entity_s *pEnt )
{
	float dt;
	vec3_t est_velocity;
...
/*
====================
StudioProcessGaitForEnt

====================
*/
void CStudioModelRenderer::StudioProcessGaitForEnt ( cl_entity_s *pEnt )
{
	mstudioseqdesc_t	*pseqdesc;
	float dt;
	int iBlend;
...
/*		if ( m_pCvarShadowsDynamic->value <= 1 )
		...
		// Highlander
		// Here, you want to prevent the p_ model
		// from being drawn, but you still call StudioRenderModel
		// you should remove this code completely, otherwise you're
...
void CStudioModelRenderer::StudioCalcAttachments( void )
{
	...
	if ( m_pStudioHeader->numattachments > MAX_ATTACHMENTS )//4
	{
...
void CStudioModelRenderer::StudioRenderFinal_Hardware( void )
{
	...
	// Highlander start
	/*
	if ( m_pCvarDrawEntities->value == 1 && m_pCvarShadows->value >= 1 && m_pCurrentEntity->curstate.renderfx != kRenderFxNoShadow && m_pCurrentEntity != gEngfuncs.GetViewModel() ) 
		StudioSetupShadows( m_pCvarShadows->value >= 2 ? false : true );*/
	// Highlander end
...
//		StudioSetupShadows();
	...
	else
	{
		for (i=0 ; i < m_pStudioHeader->numbodyparts ; i++)
		{
			...
			if( multiplayer )//no matter what
				IEngineStudio.StudioDrawPoints();

			if( (cam_thirdperson == 1) && (m_pCurrentEntity->player) )
			{
			...
			//raMp's Fix
			if ( CVAR_GET_FLOAT( "r_shadows" ) == 2 )//uses the original code
			{
				GL_StudioDrawShadow = (void(*)(void))(((unsigned int)IEngineStudio.GL_StudioDrawShadow)+32);
				HackShadows();
			}		
			if ( CVAR_GET_FLOAT( "r_shadows" ) == 1 )//draw the shadows only on certain entities and monsters
			{		
	...
	if (m_pCvarDrawEntities->value == 5)
	{
		IEngineStudio.StudioDrawAbsBBox( );//check fake collision box
	}
