...
void CCarModel :: Spawn(void)
{
	...
	//SetBodygroup( 1, 2 ); //set all bodygroups to 
...
void CEnvCar :: IdleThink()
{
	...
	fl_maxspeed = 2400;
	else if ( FClassnameIs ( pev, "car_combine_apc" ) )
	fl_maxspeed = 800;
...
LINK_ENTITY_TO_CLASS( car_combine_apc, CEnvCombineCar );
...
void CEnvCombineCar :: Precache()
{
	...
	PRECACHE_MODEL("models/cars/combine_apc.mdl");
...
void CEnvCombineCar :: Spawn()
{
	...
	pModel->pev->model = MAKE_STRING("models/cars/combine_apc.mdl");	
	SET_MODEL(ENT(pModel->pev), "models/cars/combine_apc.mdl");

	pModel->pev->origin = pev->origin;

	UTIL_SetOrigin(pModel->pev, pev->origin);

	pModel->pev->classname = MAKE_STRING("car_combine_apc_model");
	pModel->pev->solid = SOLID_NOT;
	pModel->pev->movetype = MOVETYPE_NOCLIP;
	pModel->pev->owner = edict();
	pModel->Spawn();
