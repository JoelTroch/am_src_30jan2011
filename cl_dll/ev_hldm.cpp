...
#define SLOWMO_PITCH	(float)gEngfuncs.pfnRandomFloat(45,55)
...
//void EV_FlashHit( struct event_args_s *args  );
...
float EV_HLDM_PlayTextureSound( int idx, pmtrace_t *ptr, float *vecSrc, float *vecEnd, int iBulletType )
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound( 0, ptr->endpos, CHAN_STATIC, rgsz[gEngfuncs.pfnRandomLong(0,cnt-1)], fvol, fattn, 0, SLOWMO_PITCH  );
	else
...
void EV_HLDM_FireBullets( int idx, float *forward, float *right, float *up, int cShots, float *vecSrc, float *vecDirShooting, float flDistance, int iBulletType,int iTracerFreq, int *tracerCount, float flSpreadX, float flSpreadY , float recoil)
{
	int i;
	pmtrace_t tr;
	int iShot;
...
// Recoil End
   ...
   /*     gEngfuncs.GetViewAngles( (float *)viewangles );
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/1911/1911_fire-1.wav",  gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHOTSHELL ); 
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/spas12/spas12-fire.wav",  gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
//	gEngfuncs.Con_Printf( "EV_EjectBrass\n" );
}
//======================
//	   SHOTGUN END
//======================
...
void EV_FireShotGunLessLethal( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHOTSHELL ); 
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/lesslethal/lesslethal-fire.wav",  gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, SLOWMO_PITCH );
	else
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/lesslethal/lesslethal-fire.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
...
void EV_FireMP51( event_args_t *args ) //sin sil
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
 ...
 /*   vec3_t viewangles;
    ...
    if ( EV_IsLocal( idx ) )
    {
     ...
     /*   gEngfuncs.GetViewAngles( (float *)viewangles );
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( A2_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
 	...
 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/MP5/mp5_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
//	else
//	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, SMG_RECOIL );
//	}
}

void EV_FireMP52( event_args_t *args ) //silenciada
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.10; // Cantidad de recoil 1.00
*/    // Recoil End
  ...
  */      // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SD2_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	 	...
	 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
 		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/MP5/mp5_firesil-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
 		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/MP5/mp5_firesil-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );

 
	EV_GetGunPosition( args, vecSrc, origin );

// mazor begin
EV_HLDM_MuzzleFlash( vecSrc, 0.3 + gEngfuncs.pfnRandomFloat( -0.2, 0.2 ) ); //1.0
// mazor end
...
//	else
//	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_MP5, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, SMG_RECOIL /2 );//WORKS???
//	}
}
...
void EV_FirePython( event_args_t *args )
{
	...
	//	gEngfuncs.pEventAPI->EV_WeaponAnimation( PYTHON_FIRE1, multiplayer ? 1 : 0 );
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation( PYTHON_FIRE1 + gEngfuncs.pfnRandomLong(0,1), multiplayer ? 1 : 0 ); //pfnRandomLong(0,2), 2 );

		V_PunchAxis( 0, -10.0 );
	}

		 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
 		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/357/357_FIRE-1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
void EV_FlashHit( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	{
		V_PunchAxis( 0, -2.0 );
		gEngfuncs.pEventAPI->EV_WeaponAnimation( FLASH_ATTACK1, 1 );

		switch( (g_iSwingf++) % 3 )
		{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( FLASH_ATTACK1, 1 ); break;
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( FLASH_ATTACK2, 1 ); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( FLASH_ATTACK3, 1 ); break;
...
// EVENT M16
void EV_FireM161( event_args_t *args )
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.00; // Cantidad de recoil 0.50
  */  // Recoil End
  ...
  /*      // Code Recoil Start
    ...
    */    // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M16_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
		...
		if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		{
				gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m4a1/M4a1_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
		}
		else
		{
				gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m4a1/M4a1_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		}

	EV_GetGunPosition( args, vecSrc, origin );

// mazor begin
EV_HLDM_MuzzleFlash( vecSrc, 0.5 + gEngfuncs.pfnRandomFloat( -0.2, 0.2 ) ); //1.0
// mazor end
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.00; // Cantidad de recoil 0.30
    // Recoil End
 */   
    vec3_t ShellVelocity;
    vec3_t ShellOrigin; int shell;
  ...
  */      // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M16_SIL_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2, 1 ) );
	}
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
			...
			if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
			{
					gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m4a1/M4a1_fireSil-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
			}
			else
			{
					gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m4a1/M4a1_fireSil-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
			}

	EV_GetGunPosition( args, vecSrc, origin );

// mazor begin
EV_HLDM_MuzzleFlash( vecSrc, 0.9 + gEngfuncs.pfnRandomFloat( -0.2, 0.2 ) ); //1.0
// mazor end
...
/*	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_FAMAS, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
*/		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_FAMAS, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, SMG_RECOIL );//??
//	}
}
//======================
//        AK74 START
...
//    // Recoil -Start
//    vec3_t viewangles;
//    float staerkedesrecoils = 0.00; // Cantidad de recoil 0.60 //pera un cacho (sjb)
//    // Recoil End
    ...
    VectorCopy( args->velocity, velocity );  
    AngleVectors( angles, forward, right, up );  
    shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/weapons/shell_762.mdl");
...
//        gEngfuncs.SetViewAngles( (float *)viewangles );
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( AK_NMC_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -4, -2 ) );
		//V_PunchAxis( 0, -4.0 ); //
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20, -14, 8 );

	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ak-47/AK47_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	}
	else
	{
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ak-47/AK47_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	}

	EV_GetGunPosition( args, vecSrc, origin );

// mazor begin
EV_HLDM_MuzzleFlash( vecSrc, 1.6 + gEngfuncs.pfnRandomFloat( -0.2, 0.2 ) ); //1.0
// mazor end
...
/*
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_AK74, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
	}
	else
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_AK74, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
*/			
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_AK74, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, RIFLE_RECOIL );
	
//	cl_entity_t *ent = gEngfuncs.GetLocalPlayer();

  //  angles =  ent->curstate.angles;

 //   AngleVectors( ent->angles, forward, up, right );
		
//	pParticleManager->CreateMuzzleFlash( (float *)&ent->attachment[0], Vector(0,0,0), MUZZ_AK);
//	pParticleManager->CreateMuzzleFlash( (float *)&ent->attachment[1], Vector(0,0,0), MUZZ_AK);
//	pParticleManager->CreateMuzzleFlash( (float *)&ent->attachment[2], Vector(0,0,0), MUZZ_AK);
//	pParticleManager->CreateMuzzleFlash( (float *)&ent->attachment[3], Vector(0,0,0), MUZZ_AK);
//	
//	pParticleManager->CreateMuzzleFlash( vecSrc, Vector(0,0,0), MUZZ_AK);
}

void EV_FireAK74_STAB( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	//Play Swing sound
//	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_ITEM, "weapons/ak74_miss1.wav", 1, ATTN_NORM, 0, PITCH_NORM); 

	 
	if ( EV_IsLocal( idx ) )
	{
		V_PunchAxis( 0, -2.0 );
		gEngfuncs.pEventAPI->EV_WeaponAnimation( AK_NMC_STAB, 1 );

		switch( (g_iSwing++) % 3 )
		{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( AK_NMC_STAB, 1 ); break;
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( AK_NMC_STAB, 1 ); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation ( AK_NMC_STAB, 1 ); break;
...
// Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.00; // Cantidad de recoil 0.20
// Recoil End
*/
	int empty;
...
/*		 // Code Recoil Start
  ...
  */      // Code Recoil End
		...
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? DEAGLE_FIRE1_VACIO : DEAGLE_FIRE1, 2 ); // 2 );
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/deagle/deagle_fire-1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
// mazor end
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_DEAGLE, 0, 0, args->fparam1, args->fparam2, RIFLE_RECOIL );
																				//DEAGLE
}
...
void EV_FireSniper( event_args_t *args )
{
   ...
   // shell = gEngfuncs.pEventAPI->EV_FindModelIndex ("models/weapons/shell_9mm.mdl");
    ...
    if ( EV_IsLocal( idx ) )
    {
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( SNIPER_FIRE1_PRONE + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
...
//	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/sniper/sniper_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	}
	else
	{
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.40; // Cantidad de recoil 1.00
 */   // Recoil End
 ...
 */       // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( FAMAS_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -4, -1 ) );
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 18, -14, 4 );

	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/famas/famas_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	}
	else
	{
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/famas/famas_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	}
...
/*	if ( gEngfuncs.GetMaxClients() > 1 )
	...
	{
*/		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_FAMAS, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, RIFLE_RECOIL );
//	}
}

// FAMAS BURST
void EV_FireFamas2( event_args_t *args )
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.60; // Cantidad de recoil 1.00
    // Recoil End
  */  
    vec3_t ShellVelocity;
    vec3_t ShellOrigin; int shell;
  ...
  */      // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( FAMAS_FIRE1_RND + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -5, 5 ) );
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 18, -8, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 18, -10, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 18, -12, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			...
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/famas/famas_fireburst-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );

	}
	else
	{

			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/famas/famas_fireburst-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
/*	if ( gEngfuncs.GetMaxClients() > 1 )
	{											
		EV_HLDM_FireBullets( idx, forward, right, up, 3, vecSrc, vecAiming, 4096, BULLET_PLAYER_FAMAS, 0, &tracerCount[idx-1], 0.05234, 0.05234 ); //0.03490, 0.03490,
	}
	else
	{ 											
*/		EV_HLDM_FireBullets( idx, forward, right, up, 3, vecSrc, vecAiming, 4096, BULLET_PLAYER_FAMAS, 0, &tracerCount[idx-1], 0.05234, 0.05234, RIFLE_RECOIL ); //0.08716, 0.08716
//	}
}
//======================
//	    M249 START
 ...
 */       // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( M249_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
		 	...
		 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m249/M249_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
void EV_FireUZI_SIL( event_args_t *args )
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
 ...
 //   float staerkedesrecoils = 0.20; // Cantidad de recoil
   ...
   //     viewangles[PITCH] -= staerkedesrecoils;
    //    gEngfuncs.SetViewAngles( (float *)viewangles );
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( UZI_SIL_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
...
//	V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -5, 5 ) );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
			 	...
			 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p90/p90_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/p90/p90_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );

	/*
		switch( gEngfuncs.pfnRandomLong( 0, 1 ) )
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/uzi/UZI_fireSIL-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/uzi/UZI_fireSIL-2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
	*/
...
//	if ( gEngfuncs.GetMaxClients() > 1 )
//	{
//		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
//	}
//	else
//	{
//		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
...
//	    EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
//at the  moment nop
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, 0.6 );
 ...
 //   float staerkedesrecoils = 0.20; // Cantidad de recoil
   ...
   //     viewangles[PITCH] -= staerkedesrecoils;
    //    gEngfuncs.SetViewAngles( (float *)viewangles );
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( UZI_FIRE1 + gEngfuncs.pfnRandomLong(0,1), 1 ); //pfnRandomLong(0,2), 2 );
...
//	V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -5, 5 ) );
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/uzi/UZI_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/uzi/UZI_fire-2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
...
//	    EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2 );
//at the  moment nop
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_UZI, 0, &tracerCount[idx-1], args->fparam1, args->fparam2, 1 );
...
void EV_Fire92fSil( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
/*		 // Code Recoil Start
  ...
  */      // Code Recoil End
		...
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? BER_FIRE_SIL_LAST : BER_FIRE1_SIL, 2 ); // 2 );
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			...
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ber92f/ber92f_firesil-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );

	}
	else
	{

			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ber92f/ber92f_firesil-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
// mazor end
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_BER92F, 0, 0, args->fparam1, args->fparam2, PISTOL_RECOIL /2 );//??
}

void EV_Fire92f( event_args_t *args )
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
/*		 // Code Recoil Start
  ...
  */      // Code Recoil End
		...
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? BER_FIRE_LAST : BER_FIRE1, 2 ); // 2 );
	...
	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
	{
			...
			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ber92f/ber92f_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );

	}
	else
	{

			gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/ber92f/ber92f_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
// mazor end
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_BER92F, 0, 0, args->fparam1, args->fparam2, PISTOL_RECOIL );
}
//======================
//   GLOCK18 START
//======================
...
// Recoil -Start
  //  vec3_t viewangles;
 //   float staerkedesrecoils = 0.00; // Cantidad de recoil 0.15
// Recoil End
...
//        viewangles[PITCH] -= staerkedesrecoils;
 //       gEngfuncs.SetViewAngles( (float *)viewangles );
		...
		EV_MuzzleFlash();								//last				//nah
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? GLOCK18_FIRE_VACIO : GLOCK18_FIRE, 2 ); // 2 );
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
			 	...
			 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glock18/glock18_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glock18/glock18_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
	

		
	//gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/92f_fire-1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
...
// mazor end
	...
	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_GLOCK18, 0, 0, args->fparam1, args->fparam2, PISTOL_RECOIL );

//	EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_GLOCK18, 0, 0, args->fparam1, args->fparam2 );
}

// GLOCK18 BURST
void EV_FireGlock18_B( event_args_t *args )
{
    int idx;
    vec3_t origin;
    vec3_t angles; 
 ...
 */   
  ...
  */      // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? GLOCK18_FIRE_BURST_1 : GLOCK18_FIRE_BURST_2, 2 ); // 2 );

		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -3, 2 ) );
	}

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20, -8, 10 ); //right, up, 20, -8, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20, -10, 10);//right, up, 20, -10, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

	EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 20, -12, 10);//right, up, 20, -12, 4 );
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 

			 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glock18/glock18_fireburst-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/glock18/glock18_fireburst-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
/*	if ( gEngfuncs.GetMaxClients() > 1 )
	{											
		EV_HLDM_FireBullets( idx, forward, right, up, 3, vecSrc, vecAiming, 4096, BULLET_PLAYER_GLOCK18, 0, &tracerCount[idx-1], 0.02618, 0.02618 ); //0.01745, 0.01745, 0.01745
	}
	else
	{ 											
*/		EV_HLDM_FireBullets( idx, forward, right, up, 3, vecSrc, vecAiming, 4096, BULLET_PLAYER_GLOCK18, 0, &tracerCount[idx-1], 0.02618, 0.02618, PISTOL_RECOIL ); //2 DEGREES
//	}
}
...
/*
    // Recoil -Start
    vec3_t viewangles;
    float staerkedesrecoils = 0.40; // Cantidad de recoil
    // Recoil End
 */   
    vec3_t ShellVelocity;
    vec3_t ShellOrigin; int shell;
 ...
 /*       // Code Recoil Start
   ...
   */     // Code Recoil End
		...
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation( USAS_FIRE1 + gEngfuncs.pfnRandomLong(0,0), 0 ); //pfnRandomLong(0,1), 1 );
	...
	// ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
 			 	...
 			 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/usas/usas_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/usas/usas_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
...
/*	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_USAS, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
	}
	else
	{
*/		EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, vecAiming, 2048, BULLET_PLAYER_USAS, 0, &tracerCount[idx-1], 0.08716, 0.08716, SHOTGUN_RECOIL );
//	}
}
//----------
// mp7 Event
...
//		V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -1, 2 ) );
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/mp7/mp7_fire-1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/mp7/mp7_fire-2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong( 0, 0xf ) );
		break;
	}
...
// Recoil End
    ...
    {
    case 0:
        gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/pistol/pistol_fire-1.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
        break;
    case 1:
        gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/pistol/pistol_fire-2.wav", gEngfuncs.pfnRandomFloat(0.92, 1.0), ATTN_NORM, 0, 98 + gEngfuncs.pfnRandomLong( 0, 3 ) );
        break;
    }
  ...
  */      // Code Recoil End
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
		...
		if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
		{
				...
				gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/m16/m16_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
...
void EV_FireOICW( event_args_t *args )
{
  ...
  */  // Recoil End
	...
	EV_EjectBrass ( ShellOrigin, ShellVelocity, angles[ YAW ], shell, TE_BOUNCE_SHELL ); 
 	...
 	if ( CVAR_GET_FLOAT( "slowmo" ) != 0 )
 		gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/oicw/oicw_fire-1.wav", 1, ATTN_NORM, 0, SLOWMO_PITCH );
	else
...
void EV_FireHL2ShotGunDouble( event_args_t *args )
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/spas12/spas_hl2_fire1.wav", gEngfuncs.pfnRandomFloat(0.98, 1.0), ATTN_NORM, 0, 85 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
...
void EV_FireHL2ShotGunSingle( event_args_t *args )
{
	...
	gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/spas12/spas_hl2_fire1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong( 0, 0x1f ) );
