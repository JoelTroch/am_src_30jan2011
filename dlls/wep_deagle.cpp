...
void CDeagle::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > DEAGLE_DEFAULT_GIVE) && (pev->armorvalue < 0))
		pev->armorvalue = DEAGLE_DEFAULT_GIVE;
...
void CDeagle::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/deagle/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/deagle/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/deagle/w_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( DEAGLE_DRAW, 1, 0 );	
		return TRUE;
	}
	else
	return DefaultDeploy( "models/weapons/deagle/v_deagle.mdl", "models/weapons/deagle/p_deagle.mdl", DEAGLE_DRAW, "de", /*UseDecrement() ? 1 : 0*/ 0 );
}
	...
	// ### COD RECOIL START ###
		...
		{	
			...
			{	//  si esta en el suelo y agachado
				DeagleFire( 0, ROF(700), TRUE ); //SPREAD CERO
			}
			else // si no es porq esta parado
			{																//perfe 3d
				DeagleFire( 0.01, ROF(700), TRUE );
			}
		}
...
void CDeagle::DeagleFire( float flSpread , float flCycleTime, BOOL fUseAutoAim )
{
	...
	*/
	...
	//	pev->body = (oldBody % NUM_SCIENTIST_HEADS) + NUM_SCIENTIST_HEADS * 1;//on

	//	int oldBody = pev->body;
	//	pev->body = (oldBody % NUM_SCIENTIST_HEADS) + NUM_SCIENTIST_HEADS * 0;//off
...
//		 return;
	...
	if (m_iClip == 0)
		iResult = DefaultReload( 7, DEAGLE_RELOAD_VACIO, 2.53 );
	else
		iResult = DefaultReload( 7, DEAGLE_RELOAD, 2.2 );
