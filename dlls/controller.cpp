	...
	if (m_flShootEnd > gpGlobals->time)
	{
		...
		while (m_flShootTime < m_flShootEnd && m_flShootTime < gpGlobals->time)
		{
			...
			if (m_hEnemy != NULL)
			{
				...
				if ( FClassnameIs( pev, "monster_soul" ) ) 
				pBall = (CBaseMonster*)Create( "soul_energy_ball", vecSrc, pev->angles, edict() );
				else
				pBall = (CBaseMonster*)Create( "controller_energy_ball", vecSrc, pev->angles, edict() );
...
class CMonsterSoul : public CController
{
public:
	virtual int		Save( CSave &save );
	virtual int		Restore( CRestore &restore );
...
LINK_ENTITY_TO_CLASS( monster_soul, CMonsterSoul );

TYPEDESCRIPTION	CMonsterSoul::m_SaveData[] = 
{
	DEFINE_ARRAY( CMonsterSoul, m_pBall, FIELD_CLASSPTR, 2 ),
	DEFINE_ARRAY( CMonsterSoul, m_iBall, FIELD_INTEGER, 2 ),
	DEFINE_ARRAY( CMonsterSoul, m_iBallTime, FIELD_TIME, 2 ),
	DEFINE_ARRAY( CMonsterSoul, m_iBallCurrent, FIELD_INTEGER, 2 ),
	DEFINE_FIELD( CMonsterSoul, m_vecEstVelocity, FIELD_VECTOR ),
};
IMPLEMENT_SAVERESTORE( CMonsterSoul, CController );

const char *CMonsterSoul::pAttackSounds[] = 
{
	"controller/con_attack1.wav",
	"controller/con_attack2.wav",
	"controller/con_attack3.wav",
...
const char *CMonsterSoul::pIdleSounds[] = 
{
	"controller/con_idle1.wav",
	"controller/con_idle2.wav",
	"controller/con_idle3.wav",
...
const char *CMonsterSoul::pAlertSounds[] = 
{
	"controller/con_alert1.wav",
	"controller/con_alert2.wav",
	"controller/con_alert3.wav",
...
const char *CMonsterSoul::pPainSounds[] = 
{
	"controller/con_pain1.wav",
	"controller/con_pain2.wav",
	"controller/con_pain3.wav",
...
const char *CMonsterSoul::pDeathSounds[] = 
{
	"controller/con_die1.wav",
	"controller/con_die2.wav",
};
...
//
//=========================================================
BOOL CMonsterSoul :: CheckRangeAttack1 ( float flDot, float flDist )
{
	if ( flDot > 0.5 && flDist > 256 && flDist <= 2048 )
	{
		return TRUE;
...
BOOL CMonsterSoul :: CheckRangeAttack2 ( float flDot, float flDist )
{
	if ( flDot > 0.5 && flDist > 64 && flDist <= 2048 )
	{
		return TRUE;
...
BOOL CMonsterSoul :: CheckMeleeAttack1 ( float flDot, float flDist )
{
	return FALSE;
}
/*
void CMonsterSoul ::Think( void )
{
	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
		...
		WRITE_COORD( 0 );
		WRITE_COORD( 0 );
		WRITE_SHORT(iSoulFly);
	MESSAGE_END();

	pev->nextthink	= gpGlobals->time + 0.005;
}
*/
//=========================================================
// RunAI
//=========================================================
void CMonsterSoul :: RunAI( void )
{
	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
		WRITE_SHORT(0);
		WRITE_BYTE(0);
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{
//			m_pBall[i] = CSprite::SpriteCreate( "sprites/xspark4.spr", pev->origin, TRUE );
			m_pBall[i] = CSprite::SpriteCreate( "sprites/null.spr", pev->origin, TRUE );
			m_pBall[i]->SetTransparency( kRenderGlow, 255, 255, 255, 255, kRenderFxNoDissipation );
			m_pBall[i]->SetAttachment( edict(), (i + 3) );
			m_pBall[i]->SetScale( 1.0 );
		}
...
int CMonsterSoul :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	// HACK HACK -- until we fix this.
	if ( IsAlive() )
		PainSound();
...
void CMonsterSoul::Killed( entvars_t *pevAttacker, int iGib )
{
	// shut off balls
	/*
	m_iBall[0] = 0;
	...
	*/
	...
	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
		...
		WRITE_COORD( 0 );
		WRITE_COORD( 0 );
		WRITE_SHORT(iSoulDie);
	MESSAGE_END();

	CSquadMonster::Killed( pevAttacker, iGib );
}

class CMonsterSoulBall : public CBaseMonster
{
	void Spawn( void );
	void Precache( void );
	void EXPORT AnimateThink( void );
...
LINK_ENTITY_TO_CLASS( soul_energy_ball, CMonsterSoulBall );


void CMonsterSoulBall :: Spawn( void )
{
	Precache( );
	// motor
	pev->movetype = MOVETYPE_FLY;
...
void CMonsterSoulBall :: Precache( void )
{
	PRECACHE_MODEL("sprites/null.spr");
}

void CMonsterSoulBall :: AnimateThink( void  )
{
	MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
		WRITE_SHORT(0);
		WRITE_BYTE(0);
...
void CMonsterSoulBall::ExplodeTouch( CBaseEntity *pOther )
{
	if (pOther->pev->takedamage)
	{
		TraceResult tr = UTIL_GetGlobalTrace( );
