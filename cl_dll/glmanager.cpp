...
//	if (!v_GLAllowed->value)
	...
	if (hOpengl32dll)
	{
		...
		{
			MessageBox(NULL, "VIDEO ERROR: Paranoia's hacked OpenGL32.dll is found!\n\nArrangeMent cannot run if this hacked library is present.\nRemove/rename OpenGL32.dll from Half-Life folder.\n\nPress Ok to quit game.\n", "ERROR", MB_OK);
	...
	//	Load_ARB_VBO();
		...
		// check hacked opengl library
		if ( (IsExtensionSupported("PARANOIA_HACKS_V1")) || (IsExtensionSupported("PARANOIA_HACKS_V2")) || (IsExtensionSupported("RAVEN_HACKS_V1")) || (IsExtensionSupported("RAVEN_HACKS_V2")) || (IsExtensionSupported("RAVEN_HACKS_V3")) || (IsExtensionSupported("ARRANGEMENT_HACKS_V1")) )
		{
			Paranoia_hacks = 1;
		
			gEngfuncs.Con_Printf("VidInit: Error! opengl32.dll is present!\n");

//			MessageBox(NULL, "VIDEO ERROR: Raven City's or Paranoia's hacked OpenGL32.dll is found!\n\nARRANGEMENT can not run if those hacked libraries are present.\nRemove/rename OpenGL32.dll from Half-Life folder.\n\nPress Ok to quit game.\n", "ERROR", MB_OK);
			MessageBox(NULL, "VIDEO ERROR: A hacked OpenGL32.dll is found!\n\nARRANGEMENT can not run if that hacked library is present.\nRemove/rename OpenGL32.dll from Half-Life folder.\n\nPress Ok to quit game.\n", "ERROR", MB_OK);
	...
	/*	if ( (IsExtensionSupported("RAVEN_HACKS_V1")) || (IsExtensionSupported("ARRANGEMENT_HACKS_V1")) )
		{
		}
		else
		{
			gEngfuncs.Con_Printf("VidInit: opengl32.dll is not present\n");
			gEngfuncs.pfnClientCmd("r_shadows 0\n");
		}*/

		ConLog ("Opengl hacks version: %d\n", Paranoia_hacks);
