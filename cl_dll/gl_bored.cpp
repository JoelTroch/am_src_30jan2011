...
void CBoredEffects::WaterPlane( void )
{
	...
	if( ( int )gEngfuncs.pfnGetCvarFloat( "r_water" ) == 0 )
	{
		m_blEnable = false;
	}
		...
		for( j = 0; j < 1024; j++ )
		{
			...
			if( pEnt )
			{
				if( pEnt->curstate.skin == CONTENTS_WATER )
				{
					...
					vUvOffset.x = gEngfuncs.GetClientTime( ) / gEngfuncs.pfnGetCvarFloat( "cg_water_speed_x" );
					vUvOffset.y = cos( gEngfuncs.GetClientTime( ) ) / gEngfuncs.pfnGetCvarFloat( "cg_water_speed_y" );
...
//		gTexture.CreateEmptyTex( 32, 32, m_iNormal, GL_TEXTURE_2D, GL_RGB );
...
/*
	switch ( gEngfuncs.pfnRandomLong ( 0 , 2 ) )
	{
		...
		break;
		case 1:	 	
			if( !gTexture.OverwriteTexture( m_iNormal, "noise2", 0, false ) )
			gEngfuncs.Con_Printf( "File Not Found\n" );
		break;	
		case 2:	 	
			if( !gTexture.OverwriteTexture( m_iNormal, "noise3", 0, false ) )
			gEngfuncs.Con_Printf( "File Not Found\n" );
		break;
	}*/
		if( !gTexture.OverwriteTexture( m_iNormal, "noise", 0, false ) )
