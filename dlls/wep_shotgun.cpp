...
void CShotgun::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > SHOTGUN_DEFAULT_GIVE) && (pev->armorvalue < 0))
		pev->armorvalue = SHOTGUN_DEFAULT_GIVE;
...
void CShotgun::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/spas12/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/spas12/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/spas12/w_%s.mdl", pkvd->szValue);
...
//	{
	...
	//		m_pPlayer->pev->weaponmodel = p_model;
		...
		return DefaultDeploy( "models/weapons/spas12/v_spas12.mdl", "models/weapons/spas12/p_spas12.mdl", SHOTGUN_DEPLOY, "shotgun" );
/*	}
	else
	{
	/*	bDeployed = true;
		...
		return DefaultDeploy( "models/weapons/spas12/v_spas12.mdl", "models/weapons/spas12/p_spas12.mdl", SHOTGUN_DEPLOYFIRST, "spas" );
//}*/
}
	...
	// ### COD RECOIL START ###
				...
				else // si no esta en el suelo es porque esta en el aire
				{
					...
					{// y si esta agachado en el aire gana un punto
						vecDir = m_pPlayer->FireBulletsPlayer( 8, vecSrc, vecAiming, VECTOR_CONE_15DEGREES, 2048, BULLET_PLAYER_BUCKSHOT, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
					}						// 15 puntos igual
					else
...
void CShotgun::Reload( void )
{
	...
	else if (m_fInSpecialReload == 1)
	{
		...
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.4;//0.8
	}
	else
	{
