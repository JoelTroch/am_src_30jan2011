...
#define SF_MONSTERMAKER_DONT_DROP	1024
#define SF_MONSTERMAKER_DONT_SURRENDER	1024
...
//	pevCreate->dmg = pev->dmg;//for monster breakpoint
	pevCreate->message = pev->message;//when a surrender terrorist fires a trigger
	pevCreate->target = pev->noise;

	if ( pev->spawnflags & SF_MONSTERMAKER_DONT_DROP )
		pevCreate->spawnflags		|= 1024;
//		SetBits( pevCreate->spawnflags, 1024 );//see SF_MONSTER_NO_WPN_DROP

	if ( pev->spawnflags & SF_MONSTERMAKER_DONT_SURRENDER )
		pevCreate->spawnflags		|= 2048;	
//		SetBits( pevCreate->spawnflags, 2048 );//see SF_MONSTER_DONT_REACT

/*
	if ((!strcmp((char *)STRING( pevCreate->classname ), "monster_he_grenade")))
	{
		pevCreate->Use( this, this, USE_ON, 1 );
	}*/
	...
	if (pEntity && (pMonst = pEntity->MyMonsterPointer()) != NULL)
	{
		...
		if ( FClassnameIs ( pMonst->pev, "monster_he_grenade" ) )
		{
			pMonst->Use( this, this, USE_ON, 1 );
		}
	}
