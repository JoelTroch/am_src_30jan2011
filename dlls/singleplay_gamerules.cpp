	...
	if ( FStrEq( pcmd, "menuselect" ) )
    {
		...
		if (pPlayer->m_nmenudisp == CREATE_MONSTER_AI_CLASS_ALLY)
		{
			if(!pPlayer->m_bSpawnWhitAI)
			{
				...
				if (slot == 5) 
					pEntity = CBaseEntity::Create( "cine_monster_agency_member", pPlayer->pev->origin + gpGlobals->v_forward * 128, pPlayer->pev->angles);
			}
			else
			{
				if (slot == 1) 
				...
				if (slot == 5) 
					pEntity = CBaseEntity::Create( "monster_agency_member", pPlayer->pev->origin + gpGlobals->v_forward * 128, pPlayer->pev->angles);
			}
