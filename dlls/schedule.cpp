...
//				return;
	...
	case TASK_GET_PATH_TO_SPOT:
		...
		//Sys: try to figure out where the entity is. I'll expand this code to check where are weapons
		//and the monster will try to pick them up (the same will be used to pick health kits and so on)
	case TASK_GET_PATH_TO_AI_MARKER:
		{
			/*
			CBaseEntity *pMarker = CBaseEntity::Instance( FIND_ENTITY_BY_CLASSNAME( NULL, "ai_marker" ) );
			if ( BuildRoute ( m_vecMoveGoal, bits_MF_TO_LOCATION, pMarker ) )
			{
				TaskComplete();
			}
			else
			...
			break;
			*/
			ALERT ( at_aiconsole, "TASK_GET_PATH_TO_AI_MARKER\n" );
	...
	case TASK_GET_PATH_TO_ITEM:
		{
			...
			CBaseEntity *pItem = CBaseEntity::Instance( FIND_ENTITY_BY_CLASSNAME( NULL, "weapon_9mmAR" ) );
...
case TASK_GET_PATH_TO_BESTSCENT:
	...
	case TASK_DONT_EAT:	// Don't eat - set me to not hungry for a while
		{
			Eat( pTask->flData );
			TaskComplete();
			break;
	...
	case TASK_SOUND_EAT:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) EatSound();
			TaskComplete();
			break;
		}
	case TASK_SOUND_VICTORY:
		{
			if (pTask->flData == 0 || RANDOM_FLOAT(0,1) <= pTask->flData) VictorySound();
			TaskComplete();
			break;
		}
	case TASK_WAIT_FOR_SCRIPT:
