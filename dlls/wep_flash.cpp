...
SET_MODEL(ENT(pev), "sprites/flash_beam.spr");//hack
...
void CFlash::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/flash/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/flash/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/flash/w_%s.mdl", pkvd->szValue);
...
int CFlash::GetItemInfo(ItemInfo *p)
{
	...
	p->iSlot = 0;
	p->iPosition = 2;//mostrar
    p->iFlags    = ITEM_FLAG_SELECTONEMPTY;
	p->iId = m_iId = WEAPON_FLASHLIGHT;
	p->iWeight = 10;
	p->weaponName = "Flashlight";
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( FLASH_DEPLOY, 1, 0 );	
		return TRUE;
	}
	else
...
void CFlash::UpdateSpot( void )
{
	...
	if (m_fSpotActive)
	{
		...
		UTIL_TraceLine ( vecSrc, vecSrc + vecAiming * 256, dont_ignore_monsters, ENT(m_pPlayer->pev), &tr );

//		UTIL_TraceLine ( vecSrc, vecSrc + vecAiming * 8192, dont_ignore_monsters, ENT(m_pPlayer->pev), &tr );
		
		UTIL_SetOrigin( m_pSpot->pev, tr.vecEndPos);
	}*/
#define DESERT_LASER_FOCUS			100
	...
	if (m_fSpotActive)
	{
		...
		Vector vecSrc = m_pPlayer->GetGunPosition();
		Vector vecAiming = gpGlobals->v_forward;
                    Vector vecDest = vecSrc + vecAiming * 256;
		...
		int m_iSpotBright = (1 / log(flLength / 0.3))*1000;
