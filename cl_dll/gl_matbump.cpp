...
*	(FCC)™DJShark
*	(FCC)™PROG TEAM
*   Generation Half-Life Source Code
****/
...
void PrepareRenderer( ref_params_t *pparams )
{
	...
	if (!gl.IsGLAllowed())
	{
		gEngfuncs.Cvar_SetValue( "gl_effects", 0 );
		return;
	}

	if (!gEngfuncs.GetEntityByIndex(0)->model->lightdata)
	{
		ConLog("No lightdata, Effects Disabled\n");
		gEngfuncs.Cvar_SetValue( "gl_effects", 0 );
		return;
	}
	...
	{
		ConLog("Custom effects is not supported by your videocard\n");
		gEngfuncs.Cvar_SetValue( "gl_effects", 0 );
		return;
	}
	...
	if ( CVAR_GET_FLOAT( "r_drawworld" ) != 0 )
	{
		RestoreWorldDrawing();
		renderer_prepared_for_this_frame = 1;
	}
...
//	CONPRINT("RendererDrawNormal\n");
	...
	//	CONPRINT("newframe!\n");
		...
		if (gl.IsGLAllowed() && gl.GENHL_hacks)
		{
			ResetRenderState();
		//	DrawSky();
		}
...
//	InitSky();
	...
	cv_renderer = gEngfuncs.pfnRegisterVariable( "gl_effects","0", FCVAR_ARCHIVE );
	cv_bump = gEngfuncs.pfnRegisterVariable( "gl_matbump","1", FCVAR_ARCHIVE );
	cv_detailtex = gEngfuncs.pfnRegisterVariable( "gl_detailtex","1", FCVAR_ARCHIVE );
	cv_bumpvecs = gEngfuncs.pfnRegisterVariable( "r_light_info","0", 0 );
	cv_bumpdebug = gEngfuncs.pfnRegisterVariable( "r_bump_info","0", 0 );
	cv_specular = gEngfuncs.pfnRegisterVariable( "gl_specular","1", FCVAR_ARCHIVE );
	cv_dynamiclight = gEngfuncs.pfnRegisterVariable( "gl_flashlight","1", 0 );
	cv_entdynamiclight = gEngfuncs.pfnRegisterVariable( "gl_perpixel","1", 0 );
	...
	cv_highspecular = gEngfuncs.pfnRegisterVariable( "gl_matspecular","1", FCVAR_ARCHIVE );
	...
	cv_blurtest = gEngfuncs.pfnRegisterVariable( "gl_motionblur","23", 0 );
	cv_lambert = gEngfuncs.pfnRegisterVariable( "gl_lighting","2", 0 );
