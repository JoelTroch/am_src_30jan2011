...
cvar_t	*cl_cgwater_speed_x;
cvar_t	*cl_cgwater_speed_y;
...
//	{
	// check yaw
		float ofs = viewangles[YAW] - 90;
		if (ofs > 180) ofs -= 360;
		else if (ofs < -180) ofs += 360;

		if (ofs < -10)
			viewangles[YAW] = 90 - 10;
		else if (ofs > 10)
			viewangles[YAW] = 90 + 10;
	
	// check pitch
		if (viewangles[PITCH] > 20)
			viewangles[PITCH] = 20;
		else if (viewangles[PITCH] < -20)
			viewangles[PITCH] = -20;
...
//	}
	...
	// buz: HACK - send our light level in roll angle
	//  because cmd->lightlevel is buggy thing
	cmd->viewangles[ROLL] = gLightLevel;
}
*/
...
{	
	...
	if( CVAR_GET_FLOAT( "m_angles_yaw" ) != -1 )
	{
	// check yaw
		float ofs = viewangles[YAW] - CVAR_GET_FLOAT( "m_angles_yaw" );
		if (ofs > 180) ofs -= 360;
		else if (ofs < -180) ofs += 360;

		if (ofs < -33)
			viewangles[YAW] = CVAR_GET_FLOAT( "m_angles_yaw" ) - 33;
		else if (ofs > 33)
			viewangles[YAW] = CVAR_GET_FLOAT( "m_angles_yaw" ) + 33;
	
	// check pitch
		if (viewangles[PITCH] > 32)
			viewangles[PITCH] = 32;
		else if (viewangles[PITCH] < -32)
			viewangles[PITCH] = -32;
...
gEngfuncs.pfnAddCommand ("-attack4", IN_Attack4Up);
	...
	cl_cgwater_speed_x	= gEngfuncs.pfnRegisterVariable ( "cg_water_speed_x", "2", FCVAR_ARCHIVE );
	cl_cgwater_speed_y	= gEngfuncs.pfnRegisterVariable ( "cg_water_speed_y", "5", FCVAR_ARCHIVE );
