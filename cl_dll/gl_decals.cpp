...
#include "studio_event.h" // default of mstudioevent_t
#include "r_efx.h"
#include "event_api.h"
#include "pm_defs.h"
#include "pmtrace.h"
...
//		return;
	...
	if (!pfile)
	{
		ConLog("Cannot open file \"gfx/decals/decalgroupes.txt\"\n");
		return;
	}
	...
	while(1)
	{
		...
		for (int i = 0; i < numgroups; i++)
		{
			...
			if (!numtemp)
			{
				ConLog("Warning: Empty Decal Group: %s\n", token);
				continue;
			}
...
//	ConLog("%d TGA Decal Groups Loaded in decalgroupes.txt\n", counter);
}
