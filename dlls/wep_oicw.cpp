...
LINK_ENTITY_TO_CLASS( weapon_scar, COICW );
...
void COICW::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/oicw/v_%s.mdl", pkvd->szValue);
...
//		m_pPlayer->pev->weaponmodel = p_model;
		...
		SendWeaponAnim( OICW_DEPLOY, 1, 0 );	
		return TRUE;
	}
	else
	return DefaultDeploy("models/weapons/oicw/v_oicw.mdl", "models/weapons/oicw/p_oicw.mdl", OICW_DEPLOY, "famas" );
}
...
void COICW::SecondaryAttack()
{
	...
	m_flNextSecondaryAttack = m_flNextSecondaryAttack + 0.25;
