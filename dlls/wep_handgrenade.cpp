...
void CHandGrenade::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/he/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/he/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/he/w_%s.mdl", pkvd->szValue);
...
//	PRECACHE_SOUND ("weapons/flash_bang.wav");
	PRECACHE_MODEL ("models/weapons/w_flashbang.mdl");

	PRECACHE_MODEL("models/weapons/he/w_HEgrenade.mdl"); //fix
...
*/
	if (m_fGrenMode == 2) 
	return DefaultDeploy( "models/weapons/cluster/v_cluster.mdl", "models/weapons/he/p_HEgrenade.mdl", HE_DEPLOY, "grenade" ); //"crowbar"
	else if (m_fGrenMode == 3) 
	return DefaultDeploy( "models/weapons/cluster/v_cluster.mdl", "models/weapons/he/p_HEgrenade.mdl", HE_DEPLOY, "grenade" ); //"crowbar"
	else
	{
		if (!FStringNull (v_model) )
...
//			m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( HE_DEPLOY, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy( "models/weapons/he/v_HEgrenade.mdl", "models/weapons/he/p_HEgrenade.mdl", HE_DEPLOY, "grenade" ); //"crowbar"
	}
}
...
//		flVel = ( (( 90 - angThrow.x ) * 4) * 25.0) / ( 100.0 );
//		else
...
//	return;
	...
	{	*/
		...
		else if (m_fGrenMode == 3) 
		{
			...
			ClientPrint(m_pPlayer->pev, HUD_PRINTCENTER, "#FlashBangMode"); //digamos al cliente
			Deploy();
		}
		/*
	}
