...
void CFamas::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/famas/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/famas/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/famas/w_%s.mdl", pkvd->szValue);
...
int CFamas::GetItemInfo(ItemInfo *p)
{
    ...
    p->iPosition = 2;    //FAMAS_POSITION;            // The position in a HUD slot
...
/*	if( sModifiers.iSilencer == MOD_ON )//already on, turn off
		m_iBody = 1;

	if( sModifiers.iRedDot == MOD_ON )//already on, turn off
		m_iBody = 2;

	if( sModifiers.iSilencer == MOD_ON && sModifiers.iRedDot == MOD_ON)
		m_iBody = 3;
...
//		m_pPlayer->pev->weaponmodel = p_model;
	...
	//	SendWeaponAnim( FAMAS_DEPLOY, 1, 0 );	
	//	SendWeaponAnim( FAMAS_DEPLOY );	
return DefaultDeploy( FAMAS_MODEL_1STPERSON, FAMAS_MODEL_3RDPERSON, FAMAS_DEPLOY, "famas", UseDecrement(), 0 );
	}
	else
	{
  //  return DefaultDeploy( FAMAS_MODEL_1STPERSON, FAMAS_MODEL_3RDPERSON, FAMAS_DEPLOY, "famas" );
  	//	SendWeaponAnim( FAMAS_DEPLOY );	
	//	SendWeaponAnim( FAMAS_DEPLOY, 1, 0 );	
		return DefaultDeploy( FAMAS_MODEL_1STPERSON, FAMAS_MODEL_3RDPERSON, FAMAS_DEPLOY, "famas", UseDecrement(), 0 );
	}
...
{
    if ( m_fDefaultAnim == FIREMODE_FULL || m_fDefaultAnim == FIREMODE_SEMI )
	FireSemi();
    
    if ( m_fDefaultAnim == FIREMODE_BURST )
	FireBurst();
}

void CFamas::SecondaryAttack()
{
		//TODO- CHECK IF REDDOT AND ADD ZOOM, creo
...
*/
	...
	if( sModifiers.iSilencer == MOD_ON )
	{
		...
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/m4a1/m4a1_FireSil-1.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0,0xF));
	}
	else
	EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/famas/famas_fire-1.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0,0xF));

	SendWeaponAnim( FAMAS_FIRE1 );
