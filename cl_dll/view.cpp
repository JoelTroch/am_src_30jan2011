...
void SetupFlashlight(vec3_t origin, vec3_t angles, float time, float 
...
//	gEngfuncs.Con_Printf("cross angle: %f, %f, %f\n", pparams->crosshairangle[0], pparams->crosshairangle[1], pparams->crosshairangle[2]);
...
// END
	...
	Vector lastAngles = view-> angles; //uncle misha
...
/*
	for ( i = 0; i < 3; i++ )
    {
        view->origin[ i ] += bob * 0.2 * pparams->forward[ i ];
    view->origin[ i ] += bob * 0.4 * pparams->up[ i ];
    view->origin[ i ] += bob * 0.2 * pparams->right[ i ];
    
    if( i == 3 )
        {
            view->origin[ i ] -= bob * 0.4 * pparams->forward[ i ];
        view->origin[ i ] -= bob * 0.8 * pparams->up[ i ];
...
for( int d = 0; d < 3; d++ )
    {
        view->angles[ d ] += bob * 2.0 * pparams->right[ d ];
    
    if( d == 3 )
        {
            view->angles[ d ] -= bob * -4.0 * pparams->right[ d ];
    }
    };
    view->angles[3];
    */
    ...
    {
        ///LoRes mod code
        float fl_amplitude=4; // ???????? ?????????. ??????? ????????? ? ????? ?????????.
        for ( i = 0; i < 2; i++ )
            {
                pparams->vieworg[ i ] += bob * fl_amplitude * pparams->right[ i ];
            view->origin[ i ] += bob * fl_amplitude * pparams->right[ i ];
        }
        pparams->vieworg[ROLL] -= sqrt(bob * bob) * fl_amplitude * pparams->up[ ROLL ];
        view->origin[ROLL] -= sqrt(bob * bob) * fl_amplitude * pparams->up[ ROLL ];
    }*/
...
{
	if ( CVAR_GET_FLOAT("crouch") == 1 )
	ev_punchangle[ axis ] = punch /2;
	else
	ev_punchangle[ axis ] = punch;

//	ev_punchangle[ axis ] = 0.0;
}
...
frametime)
{
	...
	gEngfuncs.pEventAPI->EV_PlayerTrace( origin, fwd, PM_NORMAL, -1, 
