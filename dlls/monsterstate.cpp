...
//POSITIONS CODE	
//POSITIONS CODE
	//SYS: horrible slow and not used
/*
	CBaseEntity *pEntityPosToCheck = NULL;
    while ((pEntityPosToCheck = UTIL_FindEntityInSphere( pEntityPosToCheck, pev->origin, 150 )) != NULL)//500
	{
		if ( FClassnameIs(pEntityPosToCheck->pev, "func_door") || FClassnameIs(pEntityPosToCheck->pev, "func_door_rotating"))
		{
			m_iPosition = NEAR_DOOR;
		}
		else if ( FClassnameIs(pEntityPosToCheck->pev, "func_breakable"))
		{	
			m_iPosition = NEAR_BREKABLE;

			if (pEntityPosToCheck->pev->playerclass == 1)
			{
				m_iPosition = NEAR_GLASS_BREKABLE;
			}
			else
			{
		...
		else if ( FClassnameIs(pEntityPosToCheck->pev, "func_ladder"))
		{
			m_iPosition = NEAR_LADDER;
		}
		else if ( FClassnameIs(pEntityPosToCheck->pev, "func_train"))
		{
			m_iPosition = NEAR_TRAIN;
		}
		else if ( FClassnameIs(pEntityPosToCheck->pev, "func_plat"))
		{
			m_iPosition = NEAR_PLAT;
		}
    }
	*/
//POSITIONS CODE	
