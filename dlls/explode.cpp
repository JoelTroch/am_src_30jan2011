...
{
	pev->velocity = RANDOM_FLOAT( 500, 800 ) * pev->angles;
	pev->velocity.x += RANDOM_FLOAT(-500.f,500.f);
	pev->velocity.y += RANDOM_FLOAT(-500.f,500.f);
...
//	pev->gravity = 0.5;
	...
	SET_MODEL( edict(), "models/weapons/he/w_HEgrenade.mdl");	// Need a model, just use the grenade, we don't draw it anyway
	UTIL_SetSize(pev, g_vecZero, g_vecZero );
	pev->effects |= EF_NODRAW;
//	pev->speed = RANDOM_FLOAT( 0.5, 1.5 );
	pev->speed = RANDOM_FLOAT( 10.5, 20.5 );
...
{ 
	...
	{
	//	pev->origin = tr.vecEndPos + (tr.vecPlaneNormal * (m_iMagnitude - 24) * 0.6);//sys: don't use this, the explosion goes up :/
	...
	if (! ( pev->spawnflags & SF_ENVEXPLOSION_NODECAL))
	{
		...
		{
			case 0:	UTIL_DecalTrace( &tr, DECAL_ST1 );	break;
			case 1:	UTIL_DecalTrace( &tr, DECAL_ST2 );	break;
			case 2:	UTIL_DecalTrace( &tr, DECAL_ST3 );	break;
			case 3:	UTIL_DecalTrace( &tr, DECAL_ST4 );	break;
		}
	}
