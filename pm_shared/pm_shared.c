...
#define PLAYER_LONGJUMP_SPEED 150 // how fast we longjump
...
//	canmakeglasssounds = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "glass" ) ) == 1 ? true : false;
	...
	*/
	...
	switch (irand)
	{
		...
		case 0:
		case 1:
		pmove->vuser3[0] = Length( pmove->velocity )/10;//8
		pmove->vuser3[1] = Length( pmove->velocity )/10;
		break;

		// left foot
		case 2:
		case 3:
		pmove->vuser3[0] = Length( pmove->velocity )/10;
		pmove->vuser3[1] = -Length( pmove->velocity )/10;
		break;
	}
}	
...
//	qboolean usingmoto = false;//sys
		...
		( speed >= velwalk || !pmove->flTimeStepSound ) )
	{
		...
		if ( pmove->flags & FL_DUCKING /*|| fWalking*/ )//sys add
		{
			fvol *= 0.35;
		}
...
void PM_WalkMove ()
{
	...
	pmtrace_t trace;
	 ...
	 ///if we are dashing (ooo)///
    qboolean  bIsDashing;
    if ( pmove->cmd.buttons & IN_DASH )
    {
         pmove->oldbuttons |= IN_DASH;
         bIsDashing = true;
    }
    else
    {
...
/*	if (wishspeed > pmove->maxspeed)
	...
	if(bIsDashing && (! ( pmove->flags & FL_DUCKING)))
    {
        if (wishspeed > pmove->maxspeed + 200)
        {
            VectorScale (wishvel, (pmove->maxspeed+200)/wishspeed, wishvel);
            wishspeed = pmove->maxspeed + 200;
        }
    }
    else
    {
...
/*	float	len;
	...
	if ( Length(punchangle) > 0.001 || Length(pmove->vuser3) > 0.001 )
	{
		...
		springForceMagnitude = PUNCH_SPRING_CONSTANT * (pmove->frametime /2);
