...
//	WALK_MOVE( ENT(pev), flYaw, m_flGroundSpeed * flInterval, WALKMOVE_NORMAL );
	...
	while (flTotal > 0.001)
	{
		...
		UTIL_MoveToOrigin ( ENT(pev), m_Route[ m_iRouteIndex ].vecLocation, flStep, MOVE_NORMAL );//test
		//MOVE_NORMAL
		flTotal -= flStep;
	}
...
{/*
	CSprite *pSprite = CSprite::SpriteCreate( "sprites/bloodspot.spr", pev->origin, TRUE );
...
{
	if ( FClassnameIs( pev, "monster_cine_dead" ) )
		return FALSE;
