...
void CM249::Spawn( )
{
	...
	if (pev->armorvalue)
	{ 
		if ((pev->armorvalue > M249_DEFAULT_AMMO) && (pev->armorvalue < 0))
		pev->armorvalue = M249_DEFAULT_AMMO;
...
void CM249::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/m249/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/m249/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/m249/w_%s.mdl", pkvd->szValue);
...
//	if( bDeployed )
	...
	//		m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( M249_DEPLOY_FIRST, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy( M249_MODEL_1STPERSON, M249_MODEL_3RDPERSON, M249_DEPLOY, "minigun" ); //"mp5"
	}
/*	else
	{
		EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/weapon_deploy.wav", RANDOM_FLOAT(0.95, 1.0), ATTN_NORM, 0, 93 + RANDOM_LONG(0,0xF));
	...
	//		m_pPlayer->pev->weaponmodel = p_model;
		...
		return DefaultDeploy( M249_MODEL_1STPERSON, M249_MODEL_3RDPERSON, M249_DEPLOY_FIRST, "minigun" ); //"mp5"
	}*/
}
