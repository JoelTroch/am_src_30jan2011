...
#define M_PI		3.14159265358979323846	// matches value in gcc v2 math.h
	...
	// buz: draw crosshair
	/*
	if ((g_vSpread[0]) && (gHUD.m_pCvarDraw->value)) // Wargon: Прицел рисуется только если hud_draw = 1.
	{
...
//		int dir = ((g_vSpread[0] * hW) / 500) * mod;
		int dir = ((CVAR_GET_FLOAT( "vuser3_0" ) * hW) / 500) * mod;
...
//		gEngfuncs.Con_Printf("received spread: %f\n", g_vSpread[2]);
		gEngfuncs.Con_Printf("received spread: %f\n", CVAR_GET_FLOAT( "vuser3_2" ));
//		int c = 255 - (g_vSpread[2] * 0.5);
		int c = 255 - (CVAR_GET_FLOAT( "vuser3_2" ) * 0.5);
