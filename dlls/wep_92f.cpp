...
void CBer92f::KeyValue( KeyValueData *pkvd )//this sets for custom fields of weapon_generic. G-Cont.
{
	if (FStrEq(pkvd->szKeyName, "m_iszModel"))
	{
		char string[64];

		sprintf(string, "models/weapons/ber92f/v_%s.mdl", pkvd->szValue);
		v_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/ber92f/p_%s.mdl", pkvd->szValue);
		p_model = ALLOC_STRING(string);

		sprintf(string, "models/weapons/ber92f/w_%s.mdl", pkvd->szValue);
...
//			m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( BER_DEPLOY_SIL, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy( "models/weapons/ber92f/v_ber92f.mdl", "models/weapons/ber92f/p_ber92f.mdl", BER_DEPLOY_SIL, "beretta", /*UseDecrement() ? 1 : 0*/ 0 );
	}
    else
	{
		if (!FStringNull (v_model) )
...
//			m_pPlayer->pev->weaponmodel = p_model;
			...
			SendWeaponAnim( BER_DEPLOY, 1, 0 );	
			return TRUE;
		}
		else
		return DefaultDeploy( "models/weapons/ber92f/v_ber92f.mdl", "models/weapons/ber92f/p_ber92f.mdl", BER_DEPLOY, "beretta", /*UseDecrement() ? 1 : 0*/ 0 );
	}
...
//    m_pPlayer->pev->effects = (int)(m_pPlayer->pev->effects) | EF_MUZZLEFLASH;
    ...
    Vector vecDir;																							
    vecDir = m_pPlayer->FireBulletsPlayer( 1, vecSrc, vecAiming, Vector( flSpread, flSpread, flSpread ), 5096, BULLET_PLAYER_BER92F / 2.5, 0, 0, m_pPlayer->pev, m_pPlayer->random_seed );
...
/*
void CBer92f::FourthAttack( void )
{
	ALERT(at_console, "FourthAttack\n" );
	
		    m_flNextFourthAttack = m_flNextFourthAttack + 0.1;
