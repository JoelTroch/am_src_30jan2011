...
LINK_ENTITY_TO_CLASS( weapon_hl2_handgrenade, CHL2HandGrenade );

void CHL2HandGrenade::Spawn( )
{
	Precache( );
	pev->classname = MAKE_STRING("weapon_hl2_handgrenade"); // hack to allow for old names
	m_iId = WEAPON_HL2_HANDGRENADE;
...
//	PRECACHE_MODEL("models/grenade.mdl"); //fix
	...
	PRECACHE_MODEL("models/hl2/weapons/v_grenade.mdl");
	PRECACHE_MODEL("models/hl2/weapons/p_grenade.mdl");
}
...
int CHL2HandGrenade::GetItemInfo(ItemInfo *p)
{
	...
	p->iSlot = 7;
	p->iPosition = 5;
	p->iId = m_iId = WEAPON_HL2_HANDGRENADE;
...
*/
	return DefaultDeploy( "models/hl2/weapons/v_grenade.mdl", "models/hl2/weapons/p_grenade.mdl", 	HANDGRENADE_DRAW, "knife" ); //"crowbar"
}
...
void CHL2HandGrenade::Holster( int skiplocal /* = 0 */ )
{
	...
	{
		// no more grenades!
		m_pPlayer->pev->weapons &= ~(1<<WEAPON_HL2_HANDGRENADE);
...
void CHL2HandGrenade::WeaponIdle( void )
{
	...
	if ( m_flStartThrow )
	{
		...
		if (m_fGrenMode == 0) 
		{
			CGrenade::ShootTimedCombine( m_pPlayer->pev, vecSrc, vecThrow, time );
		}
