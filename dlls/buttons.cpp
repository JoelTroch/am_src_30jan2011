...
LINK_ENTITY_TO_CLASS( func_deny, CBaseButton );
...
{ 
	...
	if (FClassnameIs(pev, "func_deny"))
	{
		pev->targetname = MAKE_STRING("I_WONT_OPEN");

		pev->spawnflags		|= SF_BUTTON_DONTMOVE;
...
//	MessageBox(NULL, "CFuncTButton::Spawn", "Warning", MB_OK | MB_ICONERROR);
...
/*
	if(b_in_use)
	{
		...
		sprintf(szText, "Decoding: %i", int(var*100) );
