...
void WeaponsResource :: LoadWeaponSprites( WEAPON *pWeapon )
{
	...
	sprintf(sz, "scripts/%s.txt", pWeapon->szName);
...
//	CVAR_CREATE( "vgui_dontdrawworld", "0", FCVAR_ARCHIVE ); // buz
...
//	pfile = (char *)gEngfuncs.COM_LoadFile( "menu_settings.txt", 5, NULL);
	pfile = (char *)gEngfuncs.COM_LoadFile( "scripts/menu_settings.ini", 5, NULL);
	pf2 = pfile;
	if (!pfile)
	{
...
{
	return 1;//dont draw normal panel
...
/*	if (gHUD.m_SpecTank_on)
	{
			...
			{
			//	gEngfuncs.Con_Printf("draw ammo: %d", gHUD.m_SpecTank_Ammo);
...
/*
	int iAmmo;
	iAmmo = pw->iAmmoType /= 4;

	if (pw->iAmmoType > iAmmo)
	{
...
*/
	...
	{
		if ( m_pWeapon->iId == WEAPON_FLASHLIGHT || WEAPON_HANDGRENADE || WEAPON_C4 || WEAPON_FINAL || WEAPON_RPG )
		UnpackRGB(r,g,b, RGB_YELLOWISH);
		else
		{
			r = 255;
	...
	{	
//#define	WEAPON_C4				9
		if ( m_pWeapon->iId == WEAPON_FLASHLIGHT || WEAPON_HANDGRENADE || WEAPON_FINAL || WEAPON_RPG )
		{
		}
		else
		{
...
int DrawBar(int x, int y, int width, int height, float f)
{
	...
	UnpackRGB(r, g, b, RGB_YELLOWISH);//sys
...
//	if ( !gpActiveSel )
	...
	// buz: ACHTUNG !! BUG BUG !!
	// it seems like triapi disables depthmask, making renderer messy,
	// but if render some sprite by standart methods, it becomes normal.
	// So i will render a dummy sprite

	if (gHUD.m_hsprLogo == 0)
	gHUD.m_hsprLogo = LoadSprite("sprites/%d_logo.spr");
	SPR_Set(gHUD.m_hsprLogo, 250, 250, 250 );
	SPR_DrawAdditive(0, ScreenWidth-1, ScreenHeight-1, NULL);
