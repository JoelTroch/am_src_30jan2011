...
//		UTIL_Remove( this );
	...
	//	CVAR_SET_FLOAT( "dcbbYkJY", 1 );//dialog is active (user can select choices) / EDIT; don't right now, someone is asking
...
//		pev->nextthink = gpGlobals->time + 2.5;//EDIT: 5 no es demasiado?? Hagamos la mitad
		pev->nextthink = gpGlobals->time + 5.0;//EDIT: 5 no es demasiado?? Hagamos la mitad
	}
}
...
{		
	...
	//move it here. show selection when responses are show in screen
	CVAR_SET_FLOAT( "dcbbYkJY", 1 );//dialog is active (user can select choices)

	UTIL_ShowMessage( STRING(pev->message), pPlayer );
	EMIT_SOUND(ENT(pev), CHAN_ITEM, "player/menu_on.wav", 0.9, ATTN_NORM); 

	SetThink ( ButtonsThink );//pasar al think para comprobar cada boton
	pev->nextthink = gpGlobals->time + 5.0;//pasar a responder en 5 segundos
}


void CTriggerDialog::ButtonsThink( void )
{	
	ALERT ( at_console, "CTriggerDialog::ButtonsThink, Points: %f\n", CVAR_GET_FLOAT( "TAEHeaJR" )); 
	...
	int iTotalDialogPoints = CVAR_GET_FLOAT( "TAEHeaJR" );

	if(ResponseSended == FALSE)//Si no fue enviado
	{
		if ( CVAR_GET_FLOAT( "selection" ) == 1 )
		{		
			CVAR_SET_FLOAT( "TAEHeaJR", ( iTotalDialogPoints += fh_MyRespOnePoints ) );

			//CHECK ME:
			//CHECK ME: the target is fired inmediatly! some delay must be useful to let another responses to be show
			//CHECK ME:
			...
			pev->nextthink = gpGlobals->time + 5.0;//pasar a responder en X segundos
		...
		{
			CVAR_SET_FLOAT( "TAEHeaJR", ( iTotalDialogPoints += fh_MyRespTwoPoints ) );
		...
		{
			CVAR_SET_FLOAT( "TAEHeaJR", ( iTotalDialogPoints += fh_MyRespThirdPoints ) );
...
//another entity will be fired after giving a random response (could be another trigger_Dialog)
...
{		
	...
	if ( CVAR_GET_FLOAT( "TAEHeaJR" ) >= fh_PointsNeeded)
	{
...
/*
  	if ( FStringNull( pev->target ) )
		return;
	//this points to viewangle info_target 
...
//		return;
	...
	DBSetExperiencePoints( +pev->frags );
}
...
/*	if ( !(pev->spawnflags & SF_CHEAPSUNFLARE) )
	{
		pSprite1->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxNone );
		pSprite2->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxNone );
		pSprite3->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxNone );
	}

	pSprite4->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxNone );
	pSprite5->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxNone );
*/
	//test code 2
	/*
	if ( !(pev->spawnflags & SF_CHEAPSUNFLARE) )
	{
		pSprite1->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxFadeFast );
		pSprite2->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxFadeFast );
		pSprite3->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxFadeFast );
	}

	pSprite4->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxFadeFast );
	pSprite5->SetTransparency( kRenderGlow, RedValue, GreenValue, BlueValue, LightVariation, kRenderFxFadeFast );
*/
	pev->nextthink = 0.3;
}
...
class CTextWindow : public CPointEntity
{
public:
	void Spawn( void );
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
	...
	char	m_szWindowTitle[MAX_MOTD_CHUNK];
	char	m_szTextFile[MAX_MOTD_CHUNK];
};
LINK_ENTITY_TO_CLASS( trigger_textwindow, CTextWindow );

void CTextWindow :: KeyValue( KeyValueData *pkvd )
{
	if (FStrEq(pkvd->szKeyName, "windowtitle"))
	{
		strcpy( m_szWindowTitle, pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "textfile"))
	{
		strcpy( m_szTextFile, pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else	
	{
...
void CTextWindow::Spawn( void )
{
	pev->solid = SOLID_NOT;	// Remove model & collision
	pev->effects |= EF_NODRAW;
}
void CTextWindow::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	// read from the MOTD.txt file
	int length, char_count = 0;
	char *pFileList;
	char *aFileList = pFileList = (char*)LOAD_FILE_FOR_ME( m_szTextFile, &length );

	// send the server name
	MESSAGE_BEGIN( MSG_ALL, gmsgServerName, NULL);
		WRITE_STRING( m_szWindowTitle );
	MESSAGE_END();
	...
	while ( pFileList && *pFileList && char_count < MAX_MOTD_LENGTH )
	{
		...
		MESSAGE_BEGIN( MSG_ALL, gmsgMOTD, NULL );
...
TYPEDESCRIPTION	CTextWindow::m_SaveData[] = 
{
	DEFINE_ARRAY( CTextWindow, m_szWindowTitle, FIELD_CHARACTER, MAX_MOTD_CHUNK ),
	DEFINE_ARRAY( CTextWindow, m_szTextFile, FIELD_CHARACTER, MAX_MOTD_CHUNK ),
};
IMPLEMENT_SAVERESTORE( CTextWindow, CPointEntity );
*/
